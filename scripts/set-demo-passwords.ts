  ...
  for (const u of users) {
    ...
    try {
      ...
      await prisma.user.create({
        data: { email: u.email, name: u.email.split('@')[0], passwordHash, role: 'MACHINIST', active: true },
      });
...
/**
 * One-time script to set bcrypt password hashes for demo users from Agent 1 seed.
 * Usage:
