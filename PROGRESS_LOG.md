...
- Removed the blanket drawing-number override after the owner observed `REVISION` being returned for every part. The v4.0.3 prompt now requires a contextual identifier value and forbids title-block labels as values; deterministic merge logic rejects label-only part numbers for review. Missing finish and explicit NA/N/A/NONE/NO FINISH/NOT APPLICABLE normalize to `NA`, while unreadable/conflicting finish remains unresolved.
- Shipped the accumulated server-derived order/quote decomposition plus newer importer ZIP/multi-page PDF intake, one AI request per canonical page, selected-page-only Reprocess, and no local OCR in the newer reader. No dependency, schema, migration, database, storage, customer-file or protected-config change.
- Exact release snapshot `d3131df0955fe4bce6dfd63034f37bda065b07ab`: local full suite 74 files / 408 passed / three opt-in skips; focused identity/boundary 11/11; 66-page build, type/lint and standalone copy passed. Production passed 75 files / 411 tests plus three opt-in skips and a 66-page build. One preflight attempt rejected an incomplete package baseline before downtime; no files changed. During the successful long build, the enabled monitor restarted the old runtime once; it was stopped before build completion and the final runtime started only after the new build. Add monitor disable/enable to future maintenance scripts.
- Production verification: all 86 manifest hashes passed; sampled hashes exactly match local; one runtime started 2026-09-03 13:56:49 EDT; loopback, `192.168.254.72`, and `shopapp.local` health 200; ShopApp Running (`267009` means currently running), monitor Ready/result 0; error log remains zero bytes. Rollback: `C:\ShopApp\backups\pre-update\drawing-import-release-d3131df-20260903-134939` with 29 replaced source files, prior `.next`, protected config and manifest. No DB backup was required because no data/schema mutation occurred.
- Authenticated Chrome verified saved-quote edit navigation, all three quote/order part-entry choices, new PDF/ZIP importer, local split without OCR, enabled upload after One-off selection, and order edit mode exposing Save order/Save part; zero console errors and no record/model submission. Published the audited source-only release through PR #179 and merged it into public `m4440473/shopapp1` `main`. The whole-PR audit found no customer records, uploads, drawings, exports, secrets, or production-access runbook; customer-named paths are application source/synthetic tests, and the previously tracked development DB/build-info artifacts are deleted rather than published.

### 2026-09-03 — Page-only Reprocess and no local OCR (local only)
- Confirmed bug: Reprocess reset the selected page but queued the full coordinator, which replayed all page resolution and assembly/quantity writes. Replaced with atomic page-scoped queue state (configJson.reprocessPageId), claimed by the existing recovery coordinator before any inventory. Manual retry reads only the selected canonical PDF/preview, bypasses cached/duplicate/reference short-circuits, calls the model once, and never runs packet-wide BOM/final quantity updates. Saved confirmations and previously reviewed assembly quantities stay intact; failure retains the prior review. Concurrent same-page clicks deduplicate; other-page requests wait.
- Removed local OCR invocation from the newer importer, force-disabled its old env flag, regenerated reusable local evidence from embedded PDF text instead of saved OCR guesses, and omitted supplemental local text/candidates/BOM hints from V3 AI context. PDF splitting/rendering/originals remain unchanged; no OCR dependency uninstall or legacy-importer redesign. OpenAI's direct-PDF processing still reads source PDF text and page images. No accuracy/speed claim from removing hints.
- Fixed terminal-to-retry snapshot merging so polling restarts even when completed-page count decreases; selected page keeps its spinner and prior values while processing. Other unsaved fields and stable page order remain intact. Read current target confirmations after the AI call, so an edit saved during reprocessing is retained.
- Verification: root/staged TypeScript, targeted ESLint and scoped diff checks PASS. Complete newer-importer suites: root 77 passed / three opt-in skips; staged 74 passed / three skips. Five real temporary-SQLite/mocked-AI tests per tree cover byte-identical canonical request, unchanged sibling rows and attempt history, forced reference/duplicate retry, preserved assembly quantities, failed request without automatic retry, recovery/deduplication/ownership, in-flight correction, and zero local OCR. Review UI tests 25 root / 22 staged pass. Initial root signature/schema-scaffolding/empty-SQLite setup issues corrected before final gates.
- Chrome authenticated local quote importer compiles and displays 'split locally without OCR', with upload enabled after One-off selection. No actual UI Reprocess submission, paid model call, real quote save, production write or full build; backend action verified through real service/repo with mocked OpenAI. Local runtime remains server-derived at http://127.0.0.1:3100. Next: owner retries one drawing in an existing packet; verify current UI/review results, then consider separately authorized deployment.

### 2026-09-03 — New importer ZIP/page intake and one-pass AI policy (local only)
- Owner redirected unfinished legacy prompt/schema changes to the newer importer; reverted that unfinished work and retained the prior legacy settings helper/UI. Existing newer intake already inventories mixed PDF/PNG/JPG ZIP folders and splits PDF packets locally into canonical single-page PDFs. Preserved original sources, source-relative paths, page identity/rotation and review behavior; clarified the newer panel's upload description.
- Fixed local standard-font lookup for the server-derived child checkout sharing parent node_modules. Restored the missing document worker into that checkout from a fresh read-only server download. V3 now disables automatic retries, fallback-model escalation and dimension-refinement calls, including DB settings overrides: one extraction per page, unresolved fields left for review. Independent page calls retain bounded concurrency (two locally). No dependencies, migrations or production writes.
- Verification: root/staged TypeScript and targeted ESLint pass; document suites 24 passed / one opt-in skipped each; AI suites 18 root / 19 staged; config two each. New synthetic ZIP regression covers nested duplicate filenames, multi-page vector PDFs, a scanned PDF, exact one-page model attachments, and a single failed request without retry. Broader legacy suite root 50 pass / one live-eval skip; staged 42 pass / two absent production-maintenance-script fixtures (install-shopapp-health-monitor.ps1, start-shopapp.ps1), unrelated to importer runtime. Prior settings-helper tests now execute and pass.
- Chrome access recovered: normal authenticated quote drawing panel at http://127.0.0.1:3100 shows the new description and enabled upload after One-off parts selection. No Save, new upload or paid inference; no fresh speed/accuracy measurement. No full build against the running dev tree. Next: owner tests a representative ZIP/assembly PDF locally; inspect timings and review quality before any separately authorized deployment.

### 2026-09-03 — Playground profile enabled locally for owner testing
- Implemented optional validated reasoning summary/mode in the root and server-derived AI config/adapter, with default-preservation and full-page/refinement request tests. Local trial: gpt-5.4-mini, standard mode, low effort, medium verbosity, concise summary, 10,000 output cap. Prompt, schema, image handling, shared defaults, and production are unchanged.
- Persisted `.tmp/local-browser-regression-20260903/playground.env` plus `start-local.cjs`; the hidden launcher (PID 45992 at start) runs the server-derived source at http://127.0.0.1:3100 using the existing isolated DB/storage and normal authentication. No prior port 3100 listener was running. Credentials remain in the existing root .env and are never logged/copied into the profile.
- Verification: staged baseline 16 tests; updated root 18/18 and staged 19/19; both TypeScript and targeted ESLint checks pass; launcher syntax and diff checks pass; health 200; authenticated import endpoint 200; Chrome quote drawing panel loads with no console errors and upload is enabled after choosing One-off parts. Direct API-page navigation was browser-client-blocked after server HTTP 200; verified the normal UI instead. No new drawing import or inference was run; owner performs the trial.
- Owner's PDF benchmark screenshot records 11.4 seconds, 8,087 input tokens and 1,535 output tokens. Do not claim these timings for the newly configured app until a fresh import is measured.

### 2026-09-03 — Owner's 11-second PDF extraction checked
- Owner reported an 11-second Playground extraction for `25011-00-133-602.pdf`. Read-only Poppler render (full page and enlarged title block) plus pypdf text verified part number/name, DOM tubing, zinc plate, overall 48.00 inches, OD 2.00, ID 1.25, and derived wall 0.375. Quantity is not explicitly printed; no separate stock-size callout is present.
- Limits: returned manufacturing notes omit `2X .13 X 45° CHAM.`; the bottom-right `C` is unlabeled, so do not silently label it as a revision or certify revision-null without checking the drawing convention. No same-PDF medium-reasoning comparison exists, and 11 seconds is owner-reported, not an app measurement. No configuration, application, or production change.

### 2026-09-03 — Exact drawing prompt/schema export for Playground
- Recovered the saved Responses instructions, user input, strict schema, and settings for local job `cmtllaq270014knagyihmuvtm`, response `resp_00f39bc5d9b17891006a997cdbc86087d2aa2ab0eb27e9ed47`, using read-only API retrieval and SQLite access. Actual prompt v4.0.1 differs from the root v4.0.0 quoted in the preceding response; corrected export uses the recorded version.
- Delivered `artifacts/drawing-import-playground/instructions.txt`, `user-prompt.txt`, and `schema.json`. Owner already has the image and explicitly requested only prompt/schema. No image, credentials, model call, application mutation, or deployment was included.
- Verified JSON parsing, strict `drawing_import_page_v3` schema with 17 required fields, page identity, and v4.0.1 instructions. Recorded settings: requested `gpt-5.4-mini`, resolved `gpt-5.4-mini-2026-03-17`, medium reasoning/current_turn, low verbosity, max output 10,000, served default tier. Next: await owner's Playground results before changes. Build/tests unnecessary for text-only export; script syntax and artifact checks passed.

### 2026-09-01 — Intake and shop-floor stability release deployed
- Repaired repeat-order launch as a retryable, gated dependency: templates cannot advance or submit until valid, inherited customers are shown immediately and remain replaceable/addable, and server-side creation validates the selected/inherited customer. Authenticated production browser opened a real repeat template from the list and showed WASTEBUILT preselected with no client exception.
- Added `WAITING_ON_STOCK` as a part material state (not an order status), persisted previously dropped material/drawing/procurement fields, and audit events for actual material-state changes. Shop Floor now defaults to newest-created orders and has a Summary panel for current stock waits, department movements and material arrivals.
- Phone upload now preflights the aggregate batch, uploads two photos concurrently, exposes per-photo progress/retry and waits for the session lock instead of racing. Full-resolution normalization and the existing Terra model/prompt were preserved; V2 stages now enforce route-specific concurrency, heartbeat and restart recovery rather than speculatively downgrading the model.
- Added customer-owned historical part reuse to both quote and direct-order intake. Reused parts copy only static drawing/manufacturing data, show source/version, retain drawing files, and reset quantity, pricing, procurement and assignments for explicit review. Drawing AI now extracts manufacturing notes as evidence-backed suggestions that require an explicit add action.
- Added versioned, debounced autosave/restore/discard for new direct orders and unsaved quotes, with import identity/custom-field preservation and clear-on-success. Added admin Server Monitor for app/host uptime, memory, recent order/quote/AI activity, cost/latency and active imports without exposing logs or secrets.
- Verification passed locally: TypeScript, focused zero-warning ESLint, `git diff --check`, 66-page production build, and full Vitest 79 files / 423 passed / 4 intentional skips. Server staged verification passed 15 files / 78 tests and a clean 66-page build before restart.
- Production browser verified the real repeat path/customer, fresh order's three part-entry choices and autosave indicator, Shop Floor Summary and live Server Monitor. ShopApp is Running, monitor Ready/result 0, LAN health 200, error log empty. Tailscale is Running/Online at `shopapp.tail2e8197.ts.net` / `100.69.89.39`, with HTTP 200 through both private endpoints from SHOPAPP. Rollback: `C:\ShopApp\backups\pre-update\overnight-stability-20260901-064131`.

### 2026-08-31 — Sign-in prompt classification and local phone origin fix
- AuthRequiredDialog no longer treats 403 as expired login or interrupts an order's optional locked-kiosk GET probe. Genuine protected desktop 401 still prompts; upload-only phone capability failures stay within their upload UI. No server role/auth guards changed.
- Local phone POST was rejected before auth because internal Next URL used 0.0.0.0 rather than browser Host. Compare browser-facing Host/protocol, retaining cross-site rejection. Owner's subsequent link/photo/finalize/claim all returned HTTP200; no sign-in bypass.
- 23 focused tests pass (5 prompt policy, 15 phone capability/origin, 3 phone import integration), TypeScript/lint pass. Production prompt-only release passed five tests, 63-page build, hashes and health. Rollback auth-prompts-20260831-150433; phone code remains local.
- Unresolved additional report: Repeat Orders list navigation client exception despite original-order Create again working. Both target identical /orders/new?templateId; suspected stale client scripts after build is unconfirmed. Requested hard refresh, no speculative source change.

### 2026-08-31 — Repeat-template and Create again repaired directly on production
- Both actions failed on the same snapshot write: nested part drawings lacked the required templateId. Added explicit parent identity while preserving atomic writes; top-level reads exclude part drawings, preserving correct attachment ownership for new-order prefill.
- Only production runtime file changed: src/modules/repeat-orders/repeat-orders.repo.ts. No department behavior, existing orders, schema or files were altered. Pending local search/phone/direct-order-reader features were NOT deployed.
- Verification: 24 existing service tests; 3 new real-SQLite/API tests covering snapshot, repeat reuse, new-order prefill, drawing preservation and atomic rollback; TypeScript/lint passed. Production 2-test SQLite gate and 63-page build passed, task Running and health OK. Rollback C:\ShopApp\backups\pre-update\repeat-template-20260831-145454 retains old source and build. Server source SHA256 1504B6A1ED821546EC5DA597072DFF9BBEC5A3234E0EBFF38244268289401577.
- Local preview restored at http://localhost:3001 with isolated .tmp/phone-upload-local DB/storage, normal auth. Launcher falls back to Next dev when standalone is absent; stopped only identified stale ShopApp processes, moved old cache to .tmp/local-next-before-repeatfix (recoverable). Homepage and health HTTP200. No real customer order created for testing; authenticated click-through remains owner verification.

### 2026-08-31 — Phone uploads and direct-order reader parity available locally
- Added phone-upload domain (private durable staging, hashed 30-minute upload capability, owner/draft/destination binding, count/byte/type limits, expiry/revoke, retry idempotency and partial-batch recovery). QR generated locally via qrcode; no external QR service or schema migration. Camera/gallery previews feed normalized photos into the existing canonical-PDF/V3 reader, not a new AI pipeline.
- Added admin/capability routes, phone page and compact desktop handoff. Direct orders now use the current review panel/client with destination=order instead of silently choosing legacy. Existing quote URLs remain compatible; generic admin drawing-import aliases serve both destinations. Direct-order create mapper/schema/service now retain final length/material/finish alongside dimensions and attachments.
- Verified 254 passing tests / 7 opt-in skips (includes archived fixture duplicates), TypeScript, targeted lint and production build (64 static pages). 14 phone capability tests plus 3 route/pipeline tests use disposable real schema, canonical PDFs and mocked AI; quote and direct-order destinations both reach review with expected 0.125-inch allowance. No fresh live model/photo-accuracy benchmark claimed.
- Local standalone app running normally authenticated at http://localhost:3000; phone origin http://192.168.254.132:3000. Isolated database/storage/login and synthetic fixture under .tmp/phone-upload-local, not real operational data. Launcher PID 2628; health HTTP 200 and startup error log empty. Scripts prepare-phone-upload-local.cjs/start-phone-upload-local.cjs document restart. Credentials are in local-login.json, not committed.
- Browser tool refused local sign-in with ERR_BLOCKED_BY_CLIENT. No auth bypass used. Owner-assisted PC/physical-phone UI check remains; announced this limitation. Full operational details in docs/PHONE_UPLOAD_LOCAL_REVIEW.md. Production .72 untouched, including pending global search.
- npm install reports 22 existing-tree vulnerabilities; no unrelated automatic dependency upgrades performed. qrcode added for private QR generation with @types/qrcode. Do not deploy isolated test database/credentials.

### 2026-08-31 — Business-wide search implemented locally; deployment pending approval
- Replaced order-only search with src/modules/search registry/repository/service/types and grouped Search ShopApp page. Covers orders, parts/assignments, quotes, customers/contacts, files, extracted drawing/BOM text, notes/history, catalogs/work steps, people, repeat/document templates and custom values. Both nav search labels now reflect the broader scope.
- Added multiword/punctuation-tolerant matching, category filters/counts, matched-field snippets, exact part/source links and pagination beyond the former 60-result cutoff. Search is parameterized and explicitly allowlisted; permission filtering precedes matching, counts and pagination. Credentials/configuration remain excluded, and restricted PO/quote files cannot leak via employee results.
- Verification: 26/26 tests in two search suites against a disposable database from current Prisma schema plus SSR UI; TypeScript, targeted ESLint and production build passed. Synthetic browser checked grouped cards and Next/Previous navigation with real compiled CSS. Synthetic harness intentionally omitted application font metadata/UTF-8 header, so its font/encoding differences were preview-only; harness was removed. No business records were created or modified.
- Existing live AppNav/search-page/Prisma-shim source matched local baseline before edits. No deployment performed: docs/PRODUCTION_ACCESS.md requires explicit owner production approval. Seven runtime files are ready for a narrow release; no schema migration. Preserve unrelated dirty changes.
- Limits: file names and stored extracted text are searchable, but unread scans/unprocessed binary contents/external shares are not indexed. Admin-only extracted-text results are intentionally conservative. Future scale work: measured full-text index if query-time scans become slow.

### 2026-08-31 — Read-only product usability review
- Reviewed owner/boss-man conversation requirements, live Shop Floor/Customers/Admin/Quotes screens, and source for quote editing/import review, customers, search, order controls, traveler, material workflow and attachment visibility. No application or server changes.
- Private individual-order navigation was rejected by browser approval guard; stopped that inspection without workaround. Order-detail recommendations are based on source/history, not a fresh live record walkthrough.
- Recommended priorities: whole-quote autosave with truthful save state; compact issue-first drawing review with explicit bulk edits; Shop Floor-style Quotes navigation/filters and initial-pagination repair; part-level blockers/next actions and purchasing visibility; simpler employee workspace; traveler QR deep links; unified drawing/part/PO search; explicit file roles/revisions; backup/restore visibility; later estimated-versus-actual reporting.
- Concrete source finding: admin/quotes/page.tsx fetches take=20 but initializes nextCursor:null, while client.tsx renders Load more only for a non-null cursor. This is a code-observed pagination concern, not a reproduced >20-record production failure. Existing contacts, customer filters, repeat templates, saved quote steps and durable import corrections already exist and should be extended rather than rebuilt.

### 2026-08-31 — Optional revision and in/mm dimension review deployed
- Revision now explicitly optional: absent revision no longer triggers field/page attention or missing/unreadable filters; existing save already allowed blank revision, and actual conflicting values still show evidence.
- Added a compact per-drawing in/mm toggle for finished length, width/outside diameter, thickness/wall, and derived cut/total stock. Toggle does not write data; metric edits convert to canonical inches. Cut remains final + 0.125 in (3.175 mm), total stock remains cut × quantity.
- Verified 21 current-source tests plus 9 archived-fixture tests (30 total), TypeScript, targeted ESLint, and synthetic browser interaction: blank revision saves, 50.8 mm saves as 2 in, invalid dimension blocks, corrected dimension recovers. Private production-quote browser access was denied by the tool's approval guard; no workaround was used. Used synthetic local data for UI verification instead.
- Live source hashes matched before editing; deployed only four runtime files with exact hashes. Server clean build generated 63 pages, health OK, ShopApp Running, monitor Ready/result 0, error log 0 bytes. Release SHA C97A813D10ECB4AE9DA9F23DAC1FA366904D033F06718A4361FFA31F47CDFC05. Rollback: `C:\ShopApp\backups\pre-update\drawing-review-simple-20260831-012116`.
- No schema, model, existing quote data, or document changes. Temporary synthetic harness source removed after verification. Real customer-quote browser recheck requires explicit authorization if requested.

### 2026-08-28 — Quote drawing review simplified and shop math locked (latest)
- Kept the V3 quote intake engine intact: ZIP safety, packet splitting, canonical one-page PDFs, one-page-per-model-request isolation, durable progress/evidence/reprocess, and existing quote save behavior are unchanged.
- Reworked the review into a compact BOM-analyzer-style form: removed nested colored field tiles and metric boxes, collapsed technical/supporting details, simplified filtering, restricted catalog creation to Material, and grouped fields into independent columns to eliminate blank rows caused by long warnings.
- Corrected the native Material select for dark mode with explicit background/foreground/option colors. The latest visual cleanup is live on `.72`.
- Locked manufacturing calculations locally: cut length is final length plus 0.125 inch; total stock length is cut length times reviewed quantity. AI-returned cut/stock values cannot override those rules.
- Verification passed: 32 drawing-import files / 183 tests passed with 7 opt-in skips; final focused UI/math test 9/9; TypeScript, targeted ESLint, `git diff --check`, clean 63-route local/server builds, and authenticated production review passed.
- Production is healthy: ShopApp Running, monitor result 0, empty error log, health OK. Latest rollback: `C:\ShopApp\backups\pre-update\drawing-review-simple-20260828-120339`; release SHA-256 `27167EE453D171F1C912C79B5B6087D63E57EAF98FFE9E46557E65FCFA3D0BC8`.

### 2026-08-28 — Quote Drawing Import V3 direct-PDF Terra route deployed
- Preserved V2's safe upload, durable job, vector single-page PDF, BOM/source lineage, review recovery, and quote-save framework while replacing the redundant crop-plus-page interpretation with one direct canonical-page PDF request to `gpt-5.6-terra` per eligible drawing.
- Added explicit finished length, width/outside diameter, and thickness/wall fields across strict model output, evidence, review editing, and quote-part mapping. Cut length and total stock are still computed locally from reviewed dimensions/quantity.
- Added a focused high-reasoning Terra dimension refinement only for unresolved dimensions; it cannot overwrite already resolved fields. Incomplete model responses are now forced to uncertain/manual review instead of falling back into false part creation. OCR, local auto-accept, Luna, and Sol are disabled in the V3 production beta.
- Local verification passed: TypeScript, targeted ESLint, 20 drawing-import test files / 116 tests (2 opt-in skips), and the 63-route production build. Real Terra benchmarking caught and corrected weak-local merge artifacts such as `REVISON`/`ISON`; unresolved drawing data remains explicit rather than guessed.
- Deployed 15 exact source files to `.72`, updated only non-secret V3 settings, built successfully, and restarted ShopApp. Final isolated packaged HTTP upload of the approved 9-page packet passed: 9 canonical PDFs, 9 Terra-routed pages, dimensions on all 6 classified detail pages, 3 manual-review pages, 48.626 seconds, USD 0.372434, no job error, and no production data mutation.
- Production verification: HTTP 200, ShopApp Running, monitor Ready, zero-byte error log, V3 source/config present. Rollback: `C:\ShopApp\backups\pre-update\drawing-import-v3-20260828-095926` (source, protected config, and database copy).

### 2026-08-28 — SHOPAPP Tailscale unattended access restored (latest)
- Diagnosed `shopapp.tail2e8197.ts.net` as offline even though the Automatic Windows Tailscale service and tunnel adapter were running: the backend was stuck in `NoState`, had no Tailscale IP, and its saved user profile was not active.
- A service restart alone did not recover it. DNS, outbound HTTPS to Tailscale, and WinHTTP proxy checks passed; service logs showed the saved profile repeatedly entering `NeedsLogin`/`NoState` rather than an internet outage.
- Brought the existing `shopapp` identity online with Windows unattended mode enabled, preserving `shopapp.tail2e8197.ts.net` and `100.69.89.39` rather than registering a replacement node.
- Controlled service-restart proof passed: it automatically returned to `Running` in two polls, reported Online with no health warnings, retained the same DNS name/IP, and ShopApp returned `{"status":"ok"}` through `100.69.89.39`.

### 2026-08-28 — Quote Drawing Import V2 admin beta deployed
- Replaced the admin quote drawing-intake step with a feature-flagged, durable V2 path while retaining the proven legacy importer as an immediate fallback; direct-order drawing intake remains unchanged.
- V2 safely inventories PDF/image/ZIP uploads, retains original packets and supporting CAD files, canonicalizes multi-page PDFs into vector single-page PDFs, extracts coordinate-aware local evidence/OCR, classifies pages, parses BOM rows once, computes tested assembly quantities, and routes only unresolved evidence per page through Terra with measured Sol escalation.
- Review now survives refresh/restart through durable jobs and exposes progressive stages, evidence/source regions, conflicts, page classifications, retry/reprocess/cancel actions, corrections, cost/time, and exact packet/page traceability before the existing quote save/material/quote-to-order paths run.
- Added additive migration `20260827193000_drawing_import_v2_foundation`, centralized flags/models/concurrency/budgets/pricing, strict Responses schemas, per-attempt usage/cost records, local evaluation/performance tooling, and Windows standalone native/PDF/OCR packaging.
- Verification passed: focused 23 files / 104 tests; full suite 62 files / 324 passed with 4 opt-in skips; TypeScript, targeted ESLint, 63-route build, standalone worker ping, OCR 3/3, disposable migration, local-only private 9-page ZIP smoke, and a synthetic live Terra extraction smoke.
- Deployed 72 files to `.72` from SHA-256 `CC4BDF2A3ED2A9C8E20D5B889050710347A6D83CAA30593D8B3E36E721782977`; production tests/build/worker/restart passed and all 42 migrations are current. Rollback: `C:\ShopApp\backups\pre-update\drawing-import-v2-20260828-003455` (source, database, and protected config captured before mutation).
- Final production checks passed: listener `0.0.0.0:3000`, LAN and `shopapp.local` health HTTP 200, unauthenticated V2 route HTTP 401, ShopApp Running, monitor result 0, and five representative deployed hashes match local exactly.
- This is deliberately an admin beta, not final default-release certification: local auto-accept and customer-profile matching remain disabled, the hard packet budget is USD 8, and the golden report remains `eligibleForRelease: false` until an approved representative truth set and measured real-layout performance packet are available.

### 2026-08-25 — Mobile work-order detail layout deployed
- Reflowed the work-order detail page for phones: compact horizontal Parts selection, immediate order heading, balanced two-column touch actions, full-width final Exit Order action, stacked status/priority controls, tighter mobile panels, and horizontally reachable tabs.
- Desktop remains a 360px Parts rail plus detail workspace; no order, timer, department, status, or file behavior changed.
- Local and authenticated live QA at `390x844` showed no horizontal overflow (`380px` document inside a `390px` viewport); desktop QA retained the exact 360px rail without overflow.
- Focused Vitest passed 2/2; targeted ESLint, full TypeScript, and local/server 62-page builds passed. Production IP/hostname/sign-in are HTTP 200, ShopApp is Running, monitor Ready, and error log empty. Rollback: `C:\ShopApp\backups\pre-update\mobile-order-detail-20260825-213850`.

### 2026-08-25 — Shop Floor Business quick filter deployed
- Added Business between Department and Priority with All, Sterling Tool and Die, C and R Machining, and Powder Coating options.
- Business matching is normalized and applied in the shared filtered-order pipeline, so Tiles and List always show the same result set.
- Local browser QA returned 16 All, 9 Sterling, 6 C and R Machining, and 1 Powder Coating order in both views; focused Vitest passed 15/15, targeted ESLint and full TypeScript passed, and the production build generated 62 pages.
- Deployed the exact-hash source change to `.72`; IP and `shopapp.local` health return `{"status":"ok"}`. Rollback: `C:\ShopApp\backups\pre-update\business-filter-20260825-211700`.

### 2026-08-25 — Private mobile ShopApp access confirmed
- Installed official Tailscale `1.102.3` on SHOPAPP and joined it to the owner's private tailnet as `shopapp.tail2e8197.ts.net` / `100.69.89.39` without public port forwarding or Funnel.
- The owner confirmed end-to-end ShopApp access from a phone outside the work network. This uses direct private tailnet access; Tailscale Serve/HTTPS is not enabled and is not required for the confirmed path.

### 2026-08-25 — Print-header parity and aligned Parts rail deployed
- Fixed the browser-print defect that hid the quote business header: global print CSS now hides only explicitly marked ShopApp navigation/footer chrome, not every semantic document header/footer.
- Aligned the order-detail `PARTS` title with the large customer title, aligned it with the Parts subheading/card rail, and removed the translating hover that made part cards wiggle under neighboring content.
- Local browser measurement confirmed both titles at 24px, a 2px baseline difference, identical Parts/subheading x-position, and a stationary hovered card with computed transform `none`.
- Focused Vitest passed 4 files / 10 tests; targeted ESLint and full TypeScript passed. After explicit approval, the hash-checked `.72` build generated 62 pages and the service restarted successfully.
- Production verification passed through both the IP and `shopapp.local` health endpoints plus sign-in; ShopApp is Running, Health Monitor is enabled, and the error log is empty. Rollback: `C:\ShopApp\backups\pre-update\print-header-order-layout-20260825-183246`.

### 2026-08-25 — Held feature batch deployed and production work history reset
- Deployed the locally reviewed order-detail tile, traveler role, newest-first Shop Floor, customer multi-contact/structured-address, multi-worker assignment, business document-header, disclaimer-block, and `DDMMYY-###` quote-number changes to `.72`.
- Created rollback snapshot `C:\ShopApp\backups\pre-update\fresh-start-20260825-154601`, then applied migration `20260825150000_customer_contacts_structured_address_v1` and completed the 62-page production build.
- Preserved exact production records `CRM-1007`, `CRM-1008`, and quote `250826-001`; removed 16 older orders and 17 older quotes through a guarded transaction after the same script passed against a disposable production copy.
- Customer/contact/user/department/document-template/settings/repeat-template counts stayed unchanged, physical customer files were not deleted, and SQLite returned zero foreign-key issues.
- Production verification passed: IP and `shopapp.local` health plus `/auth/signin` returned HTTP 200, selected deployed hashes match local, ShopApp is running, the health monitor is enabled/healthy, and `shopapp.err.log` is empty.

### 2026-08-25 — Defined Shop Floor-style order part tiles
- Restyled order-detail part cards with the same defined navy/royal-blue visual hierarchy as Shop Floor tiles: selected parts use the brighter royal-blue surface and unselected parts retain a clearly bordered deep-navy surface.
- Strengthened part-card text contrast while preserving all existing selection, department, timer, and order behavior.
- Verification passed: lint/type-check, 23/23 focused tests, clean local/server 62-page builds, local visual/interaction QA, and authenticated live selection QA.
- Deployed to `.72`; health is HTTP 200, source hash matches, port 3000 is listening, error log is empty, rollback is `C:\ShopApp\backups\pre-update\defined-part-tiles-20260825-1420`, and DB/storage invariants were preserved.

### 2026-08-25 — Open order canvas and unassigned department path (latest)
- Removed the dark outer Parts card and selected-order workspace card so both columns sit directly on the navy page canvas; individual part tiles and functional inner panels remain intact.
- Fixed the unassigned-part dead end: the department action is enabled and labeled Assign department when no current department exists, then retains Move department wording after assignment.
- The shared audited dialog now uses assignment-specific title/help/reason text for unassigned parts, offers all active departments, requires a note, and continues using the existing manual department service.
- Verification passed: lint/type-check, 23/23 focused tests, clean local/server 62-page builds, local visual QA, and authenticated live dialog QA without saving a production mutation.
- Deployed to `.72`; health is HTTP 200, source hash matches, error log is empty, rollback is `C:\ShopApp\backups\pre-update\order-open-canvas-20260825-1345`, and DB/storage remained intact.

### 2026-08-25 — Shop Floor Timers and department quick filter (latest)
- Promoted Timers into its own top-level Shop Floor view control beside Tiles, List, and More; it independently reveals Working Now while More now owns customization only.
- Added Department between Status and Priority with All, Unassigned, and every active department; matching uses each order's effective current part departments and applies to both Tiles and List.
- Verification passed: targeted lint/type-check, 14/14 focused tests, clean local/server 62-page builds, local visual QA, and authenticated live interaction QA.
- Deployed the single component to `.72`; health is HTTP 200, source hash matches, error log is empty, rollback is `C:\ShopApp\backups\pre-update\shopfloor-timers-department-20260825-1310`, and production DB/storage were preserved.

### 2026-08-25 — Admin order controls, HOT indicators, and tile actions (latest)
- Added obvious admin-only Priority and audited Change status controls to the order header; manual status overrides require a reason and use the existing history-backed status service.
- Added HOT flame indicators in Tiles and List plus an admin-only three-dot tile dialog for priority, status, and assigned-machinist updates.
- Removed priority from the broad Edit order payload so a stale edit form cannot overwrite the new focused priority control.
- Verification passed: targeted lint/type-check, 35/35 focused tests, clean local/server 62-page builds, and local plus authenticated production interaction QA without saving test mutations.
- Deployed to `.72`; health is HTTP 200, source hashes match, error log is empty, rollback is `C:\ShopApp\backups\pre-update\order-controls-20260825-1245`, and DB/storage invariants remain unchanged.

### 2026-08-25 — List-first summary and compact workload refinement (latest)
- Moved the four dashboard summary cards inside List mode directly above the order table; Tiles and More no longer lead into the old page-bottom dashboard.
- Replaced Completed jobs with an equal-height compact Machinist workload card showing the top three active assignment counts with TV-readable spacing.
- Removed the duplicate lower Orders overview, full Machinist workload, and Status pulse blocks so List has one summary and one table only.
- Verification passed: targeted ESLint, TypeScript, 14/14 focused tests, clean local/server 62-page builds, local and authenticated production visual QA, and live HTTP 200 health/sign-in.
- Deployed the same two Shop Floor source files to `.72`; hashes match, rollback is `C:\ShopApp\backups\pre-update\shopfloor-list-summary-20260825-1210`, and the production DB/storage invariants remain unchanged.

### 2026-08-25 — Shop Floor Tiles/List/More production landing view (latest)
- Replaced the large Shop Floor introduction and initially expanded timer/customization hierarchy with a compact filter row followed immediately by the requested three-column order tiles.
- Added adjacent Tiles, List, and More controls: Tiles is the fresh-load default, List renders the same filtered/sorted orders in a dense overview table, and More reveals Working Now plus the full customization controls.
- Preserved advanced filters, conditional tile colors, and saved TV configuration while removing the redundant old layout selector from the customization panel.
- Verification passed: targeted ESLint, TypeScript, 14/14 focused tests, clean local and server production builds with 62 pages, `git diff --check`, and authenticated live interaction/visual QA at `shopapp.local`.
- Deployed only `src/app/page.tsx` and `src/components/ShopFloorLayouts.tsx` to `.72`; source hashes match, rollback files are under `C:\ShopApp\backups\pre-update\shopfloor-view-20260825-1205`, health/sign-in return HTTP 200, and the unchanged production database/storage invariants are 1,142,784 bytes plus 12 files / 2,191,862 bytes.
...
- Verification passed: targeted ESLint, TypeScript, live save/reload verification, and visual resolved-state inspection.

### 2026-07-17 — Quote part-list overflow correction (latest)
- Corrected the quote editor's manual parts sidebar after long imported names, part numbers, and quantities escaped the fixed-width list and overlapped the selected-part form.
- Widened the responsive list track, gave both grid columns safe shrink behavior, and changed each shared-Button row from inherited single-line horizontal layout to an auto-height stacked/wrapping layout.
- Verification passed: targeted ESLint, TypeScript, and live browser QA with deliberately oversized text. Measured row descendants remain inside the panel, row scroll width equals client width, and the page has no horizontal overflow.

### 2026-07-17 — Production feeds-and-speeds correction for Haas VF-2SS (latest)
- Reworked the calculator around an explicit Haas VF-2SS profile with 12,000 RPM and 833 IPM hard programming ceilings; uncapped targets remain visible in warnings instead of leaking into programmed output.
- Added deliberate tool-family defaults and the missing operation inputs for stickout/flute length, shank diameter, lead angle, corner radius, work diameter, thread diameter/location, tap lead, and explicit slotting.
- Ported the uploaded FSWizard geometry/deflection behavior for specialized milling, corner rounding, thread milling, drilling/reaming, tapping, turning, and grooving. Removed fabricated ramp/plunge outputs and replaced the hardness-only torque indicator with calculated MRR, horsepower, and correctly dimensioned torque where supported.
- Replaced self-generated snapshots with explicit source-derived expectations and expanded coverage across defaults, overrides, machine caps, invalid geometry, all 29 tool families, and representative specialized branches.
- Verification passed: focused tests `19/19`, full tests `140/140`, full lint, TypeScript, production build with 59 generated pages, and live browser QA for default/reset, V-bit RPM cap, tap-pitch validation, thread-mill validation, styling, and console health.

### 2026-07-16 — Drawing import progress and removal follow-up
- Replaced the indefinite blue spinner with a staged progress bar that advances while extraction runs and displays status, percentage, and elapsed seconds.
- Added `Remove from list` to every imported drawing review card, including assembly detections, and remove actions update both proposal/review state before order creation.
- Verification passed: drawing-import tests (3/3), full lint, and live side-browser loading of the updated import panel.

### 2026-07-16 — Drawing-to-order import v1 (latest)
- Implemented dedicated part names plus reviewed PDF/image/ZIP-assisted order creation, per-part drawing attachments, and automatic BOM startup.
- Verification passed: Prisma migration/generation, 25 focused tests, full lint, 16-file sample ZIP expansion, live `/orders/new` compilation, and representative title-block extraction.
- Full implementation details, known follow-ups, and command evidence are recorded in the matching detailed entry below and in `docs/AGENT_HANDOFF.md`.
...
### 2026-08-24 — Windows LAN production deployment operational (latest)
- Deployed the current ShopApp working state to Windows 11 Pro at `192.168.254.72` under separated `C:\ShopApp\app`, `config`, `data`, `storage`, `logs`, `backups`, and `maintenance` directories; installed Node 22 and Git, preserved the current SQLite data and 12 attachments, applied 38 migrations, and produced a clean 62-page standalone build.
- Registered ShopApp as a SYSTEM startup task with no stored administrator password. After the first reboot exposed a Windows network-profile firewall gap, installed a reusable boot supervisor, made firewall access profile-independent but address-scoped, and enabled NLA RDP as an independent fallback restricted to the admin workstation.
- Confirmed the router-reserved `.72` address is delivered by DHCP to MAC `D8-9E-F3-16-CD-4A`. Added persistent IPv4/IPv6 port-80 forwarding so the LAN site works at `http://desktop-bkbakpm.local` without a port number; a future onsite computer rename can shorten this to `http://shopapp.local`.
- Verification passed for archive/database hashes, dependency install with seeding disabled, migrations/generation/build, LAN health/sign-in, SSH/RDP/ShopApp ports, immediate boot-supervisor execution, 13 users / 4 customers / 16 orders / 64 parts, 12 attachments, and the pre-deployment database backup.
- Remaining: repeat a full reboot with an operator onsite now that the corrected supervisor exists; configure `.10` secondary backups later; separately address the existing npm/Next.js security advisories.
...
### 2026-07-16 — Drawing-to-order import workflow design
- Inspected the current order builder, part attachments, canonical file storage, persisted BOM analyzer flow, and the supplied `fwdfiles.zip` sample.
- Confirmed the ZIP contains 16 PDFs with strong title-block identity data and at least one multi-page assembly drawing with a parts list.
- Added `docs/DRAWING_ORDER_IMPORT_WORKFLOW.md`, covering the low-friction operator flow, confidence-based review, assembly/duplicate handling, partial failures, suggested module boundaries, delivery slices, and first-release acceptance criteria.
- No application code, schema, dependencies, or supplied drawings were changed.

Verification evidence:
- ZIP entry inventory completed: 16 PDF files.
- Representative PDF text extraction confirmed drawing number, part name, material, and assembly parts-list content.
### 2026-07-16 — Drawing-to-order import v1
- Added a dedicated nullable part-name field to live order parts and repeat-order template parts, including migration, create/edit/quote-conversion/repeat-order flows, search, order detail, and print display.
- Added a drawing-import module and admin upload API for PDF/PNG/JPG or bounded ZIP input (50 files, 20 MB each, 100 MB expanded total, CRC checking, traversal-safe filenames).
- Added title-block extraction for part number, name, quantity, material, stock size, cut length, revision, assembly flag, evidence, and confidence; unavailable/invalid extraction falls back to filename identity and highlighted manual confirmation.
- Updated `/orders/new` with large manual-vs-drawing choices, simple upload progress, editable review cards, material matching, explicit quantity fallback, assembly warnings, per-part drawing links, and final readiness summary.
- Imported drawings now create per-part attachments, normalize into canonical order storage, and automatically start the existing BOM analyzer against returned created-part IDs with bounded concurrency; analyzer failure does not roll back the order.
- Added `jszip` and recorded the dependency/architecture decision in `docs/AGENT_CONTEXT.md`.

Verification evidence:
- `npx prisma migrate dev` applied `20260716103000_add_order_part_name`; Prisma client generation passed.
- Focused drawing-import, Orders, repeat-order, and quote-conversion suite passed: 25/25 tests.
- `npm run lint` passed with no warnings/errors.
- Real `fwdfiles.zip` verification found 16 supported PDFs totaling 1,049,086 expanded bytes.
- Live `/orders/new` returned 200; drawing API returned a deterministic 400 for malformed multipart input.
- Live representative-PDF extraction returned part `26031-00-133-602`, name `STANCHION TUBE`, quantity uncertainty, material text, and cut length.
- `npx tsc --noEmit --pretty false` remains blocked by previously documented baseline errors outside this feature; no drawing-import/part-name path errors were reported.

Next:
- Add persisted/resumable import drafts if users need to leave during large imports.
- Add explicit assembly parts-list component expansion (assembly only / listed components / both); v1 flags assemblies for manual confirmation.
### 2026-07-16 — Neon-orange confirmation palette + quantity checkbox (latest)
- Replaced the confirmation UI's amber/yellow/brown treatment with white cards, neon-orange (`#ff5a00`) attention borders/glow, and navy (`#0b1f3a`) status/confirmation surfaces.
- Resolved tiles now use a quiet white/navy border instead of green/yellow competition, and extraction warnings use an orange rule with navy text.
- Uncertain quantities now show a direct `Quantity N is correct` checkbox; checking it clears the quantity reason without requiring a number-input arrow/edit.
- Quantity was removed from the bulk confirm button so the dedicated checkbox is the obvious, low-effort action.
- Verification passed: focused suite (13/13), targeted ESLint, full lint, and live review inspection. The owner's confirmed 15-part + 1 assembly-file import remained intact.

### 2026-07-16 — Live per-field drawing confirmation highlights (latest)
- Replaced the import-time `needsReview` latch with confirmation reasons derived from current values, extraction confidence, and explicit confirmations.
- Only unresolved part tiles are yellow; each yellow tile now lists the exact fields/reasons and highlights only those field containers.
- Editing a highlighted field clears its warning when valid. Uncertain prefilled values have a plain `The highlighted values look right` action, and assembly detections have an explicit `Keep as a part` decision alongside the existing keep-as-order-file path.
- The header changes from `# need confirmation` to `All parts confirmed` as issues are resolved, and Continue unlocks from the same live state.
- Verification passed: focused suite (13/13), targeted ESLint, and full lint. Live inspection preserved the owner's existing 15-part + 1 assembly-order-file review form rather than restarting it.

### 2026-07-16 — Finish, assembly-file, and material matching follow-up (latest)
- Added FINISH to title-block extraction and review; the original finish remains visible/editable and becomes a `Finish: ...` line in the created part notes.
- Assembly detections now offer `Remove from part list; keep file`, move into a clear order-files section, remain attached at order level, and can be restored to the part list if detection was wrong.
- Replaced literal material matching with shared shop-name/grade normalization covering catalog shorthand such as 6061, C.R.S./cold-rolled steel, 304 stainless, and PTFE/Teflon; review shows both the raw drawing value and matched catalog material.
- Verification passed: 12 focused tests, targeted ESLint, full lint, and live page compilation. Full TypeScript produced only the documented baseline errors outside the touched paths. The browser session required sign-in after reload, so the owner-facing ZIP click-through remains the final manual check.

### 2026-07-16 — Drawing review preview fix (latest)
- Fixed Open drawing during pre-order review by adding an authenticated, admin-only draft preview endpoint; persisted order attachments continue using the existing attachment route.
- Added storage-root containment, traversal rejection, supported-drawing validation, inline response headers, and no-store caching for draft previews.
- Updated both drawing-review Open drawing links to use the draft-aware preview path.
- Verification passed: focused drawing-import tests (4/4), targeted ESLint, full lint, and a live authenticated sample-ZIP PDF response with `application/pdf` instead of 404.

### 2026-07-16 — Latest drawing-import test order cleanup (latest)
- Identified CRM-1004 for Versa Tech Automation as the newest order by received date/order number and confirmed it contained 16 imported parts.
- Deleted CRM-1004 in a database transaction, including 16 part drawing records, 16 parts, one BOM analysis, and its status history.
- Removed only the verified canonical CRM-1004 drawing folder under the Versa Tech Automation storage path.
- Verified CRM-1004 and its canonical folder are absent, STD-1007 for Toyota-TMMK is now the newest remaining order, and the running `/orders/new` page still returns 200.

### 2026-07-16 — Quote-first resumable workflow (latest)
- Made Quotes the admin-only starting point for the pre-production flow without representing drafts as Orders.
- Added five persisted checkpoints: Customer, Drawings & parts, Material check, Work details, and Pricing & finish. **Save & continue** writes the draft before advancing; reopening uses the saved checkpoint and Quotes list rows show a resume link.
- Reused the drawing importer in Quote Editor. Imported detail drawings belong to their quote parts; removed assemblies remain quote-level files; exact material/finish wording remains visible alongside normalized material selection.
- Added per-part material disposition, stock location, purchasing note, selected vendor, finish, drawing wording, stable sort order, and quote-part attachments.
- Added a pricing-free admin print page for the shop material walkdown with large handwritten checkboxes/lines.
- Updated conversion to copy all new operational fields and part drawings, set order material-needed state, keep quote price/custom-price rows off the order, record `Order.sourceQuoteId`, and prevent duplicate conversion at the database layer.
- Quote-owned attachments are now admin-only through the attachment route. BOM analysis now requires authentication and uses one normal model pass plus at most one targeted tolerance fallback instead of five unconditional passes.
- Added `docs/QUOTE_FIRST_WORKFLOW.md` with the workflow/data/conversion/cost architecture and official pricing references.

Verification evidence:
- Applied migrations `20260716154035_quote_first_workflow_v1` and `20260716155142_quote_source_provenance_v1`; Prisma generation and validation passed.
- Focused quote/conversion suite passed 9/9.
- Targeted ESLint passed for all touched quote, order, drawing, print, attachment, and analyzer files.
- Live port-3000 checks rendered quote new/list/edit and material-walkdown print pages; `/admin/quotes/new` remains open for owner testing.
- Full TypeScript still reports only the documented repo baseline errors; no quote-first files are in the error list. The related pre-existing print-analyzer canvas typing error was fixed.

Next:
- Add source-hash/analyzer-version caching and response-usage telemetry before evaluating local OCR or a cheaper model on a real corrected-drawing test set.
- Add a dedicated quote BOM-analysis record if admins need machining-feature analysis visible before conversion; do not rerun it during conversion.
### 2026-07-16 — Quote workflow simplification, pricing integrity, and admin cleanup (latest)
- Reorganized Admin around daily work: large `New Quote` and `Resume Quotes` actions, compact navigation, clearly grouped setup, and direct order creation demoted to emergency/internal jobs.
- Added one shared server-side `/admin` guard plus signed-out/non-admin/admin regression coverage.
- Replaced operator-facing add-on/checklist language with `Work Steps` and one usage choice while retaining the legacy database/API names for compatibility.
- Frozen selected Work Step name/rate/department/price-shop behavior on each quote, added quote-part/step uniqueness, and removed two legacy duplicate groups in migration `20260716161000_quote_work_step_snapshots`.
- Made final part-price intent explicit (`CALCULATED` or `MANUAL`), supported intentional $0 prices, matched pricing to stable quote-part IDs, and stopped work estimates from being added again after a final part price is chosen.
- Simplified quote intake/customer ownership, material notes, customer print pricing, approval status, and the single quote-detail conversion review. Conversion carries quote material/vendor/shop/file data and no longer asks for it again.
- Verified the live task-first admin home, Work Steps list/dialog, and five-checkpoint quote intake; left the running app at `/admin/quotes/new` for owner testing.

Commands/evidence:
- `npx prisma migrate status` — 32 migrations found; database schema up to date.
- focused Vitest command covering admin layout, quote pricing/work steps, approval, conversion, and quick-convert UI — 7 files / 22 tests passed.
- targeted ESLint across changed admin/quote/pricing files — passed with no errors.
- `npx tsc --noEmit` — no task-related errors; still blocked by the logged vendor-import, kiosk, order-test, mock-repo, and `sterling-site` baseline errors.
- `npm run build` — task code compiles; repo-wide type check remains blocked first by the pre-existing vendor-import `existing.id` typing error, with the pre-existing kiosk schema import warning also present.
- Live development server is running on `http://127.0.0.1:3000`; `TEST_MODE=true` intentionally supplies a mock admin for local browser testing.

### 2026-07-17 — Kiosk / employee / department / time-flow audit
- Reproduced the production type-check baseline and separated localized compile drift from runtime shop-floor integrity risks.
- The build blockers are small and independent: vendor-import tuple inference, one obsolete kiosk route/import, kiosk generic result typing, stale Orders tests, mock-repository contract drift, and the standalone `sterling-site` being included in the root type-check.
- Audited normal authentication, kiosk PIN authentication, shared-station worker identity, department ownership/transitions, checklist submission, timer interval lifecycle, corrections, totals, and current database state.
- Found P0 runtime risks: kiosk-disabled users can still authenticate; deactivated/role-changed users keep stale JWT access; single-active timers are not database-enforced; close-and-switch is non-atomic; legacy timer APIs allow missing or mismatched parts; and visible department movement bypasses the canonical completion rules.
- Found a current active timer whose recorded department differs from the part's current department, demonstrating that moves/completion can strand active labor.
- Found the canonical final-department submit returns `COMPLETE` but persists `IN_PROGRESS`, while ordinary checklist completion can prematurely mark an order complete.
- Found admin time correction cannot edit another employee's interval and lacks transactional before/after audit data; manual stored-seconds adjustments also produce totals that disagree with interval reports.
- Found overlapping timer API families, two floor timing surfaces, dead order-page timer handlers, legacy completion routes, and three actor identities across timer/checklist/department actions.
- No application behavior or database records were changed during this audit.

Verification evidence:
- `npx tsc --noEmit --pretty false` reproduced the documented baseline errors.
- Focused kiosk/time/orders suite passed 25/25; coverage gaps are documented in `tasks/todo.md`.
- Read-only data checks: 55 intervals, one active timer, no current duplicate-active users, seven active kiosk-disabled users retaining PIN hashes, and five parts with null current-department ownership.

Next:
- First implement a bounded security/integrity slice: kiosk eligibility and revocation, session freshness, database-enforced single-active timers, atomic switching, and audited admin stale-timer recovery.
- Then consolidate department movement around one canonical submit/override contract, fix completion persistence/status derivation, block active-timer moves, and retire legacy routes.
- Keep the build-baseline cleanup as a separate small no-behavior-change ticket.

Owner-direction addendum:
- Confirmed the large-TV computer is a trusted interactive dispatch console for five to seven employees per department, not an employee-by-employee login kiosk.
- The console operator must control timers on behalf of workers; the system will record console actor and labor owner separately.
- Chosen invariant is one active timer total per employee, with atomic pause-and-switch for urgent interruptions and multiple workers allowed on one part.
- Part assignment remains intact while paused. Timer state does not move or complete the part.
- Part History/Log will group interval-derived labor by employee and show employee subtotals plus the complete part total.

### 2026-07-17 — Trusted shop-floor dispatch console completed
- Replaced the normal floor timer path with one trusted-console workflow: the selected employee owns the labor interval and the signed-in console user is recorded separately for every start, switch, pause, finish, and correction.
- Enforced one open timer total per employee with a partial unique database index and atomic pause-and-switch transactions. Multiple employees can still work the same part.
- Preserved part assignments during pauses and switches; department movement/completion is independent and refuses to proceed while a part has active timers.
- Added a large `Working now` strip to the TV dashboard with live duration, employee, order, part, and department; department queue rows show assigned workers and both dashboard feeds refresh automatically.
- Added part Timer History grouped by employee with employee subtotals, all-worker total, live running time, interval/action detail, and console actor attribution.
- Consolidated the optional PIN kiosk onto the same atomic timer service, enforced kiosk enablement/session expiry, refreshed normal user activity/role at the session boundary, and removed weaker legacy mutation/auto-advance routes.
- Fixed final department completion persistence, premature order completion derivation, admin cross-employee time correction, and before/after correction auditing.
- Backfilled the one active part missing a department so active work no longer disappears from the TV queue; completed historical null-department records were intentionally preserved.
- Browser QA found and fixed a sticky `?part=` bug that prevented switching away from a dashboard-linked part. Part selection now changes controls, history, and the shareable URL together.
- New timers are blocked for Shipping or completed parts while any pre-existing stale timer remains visible and can be paused/finished.
- Replaced the Bash-only standalone copy step with a cross-platform Node script so the production build completes on Windows and Docker/Linux.

Verification evidence:
- Migrations applied: `20260717120000_trusted_dispatch_timers`, `20260717124500_backfill_active_part_departments`.
- Full ESLint and `npx tsc --noEmit` passed.
- Full Vitest suite passed: 28 files / 127 tests.
- `npm run build` passed: compile, type/lint validation, all 59 static pages, route tracing, and standalone asset copy.
- Live browser QA passed at 1920x1080 for dashboard, assignment labels, direct worker-to-part navigation, part switching, trusted timer dialog, Pause/Finish choices, employee-grouped History/Log, and Shipping/complete timer guard.

Operational note:
- The existing Jim timer on CRM-1002 / QA-2-1 is roughly 1,555 hours old and conflicts with the part's completed/Shipping state. It was intentionally not altered without owner confirmation; the new UI exposes Pause and Finish for cleanup and prevents any new timer on that part.

### 2026-07-17 — FSWizard feeds-and-speeds math audit
- Located and audited the owner-provided FSWizard calculation source at `C:\Users\user\Downloads\this.go`, the decoded embedded database, the ShopApp calculation module, UI, parity notes, tests, and live page.
- Confirmed the app-owned database is an exact copy of the decoded upload: 34 materials, 29 tool types, 5 tool materials, 9 coatings, and 68 chipload rows.
- Confirmed the ordinary SFM/RPM/chipload stack, drill helix/feed formula, and pitch-driven tap feed are substantially faithful to the source when the required inputs are correct.
- Found a critical missing machine-speed limit: the original source defaults to a 10,000 RPM cap, while ShopApp can recommend 162,014 RPM for the bundled High Feed Mill and 129,932 RPM for the V-bit.
- Found specialized parity gaps in high-feed/lens/chamfer geometry, the separate corner-rounding area model, thread-mill circle/internal/external compensation, turning/grooving load/deflection, and tap thread/drill selection.
- Found secondary-output defects: the maximum-angle ramp-feed endpoint is treated as a constant factor, drill/ream feed is duplicated as an invented 60% “plunge” feed, absolute negative DOC defaults are rescaled by tool diameter, and thread-mill pilot size is based on tool diameter rather than thread circle diameter.
- Live verification confirmed the initial Solid End Mill family default is 2 flutes but the page initializes at 4; the current 4140 example therefore displays 35.39 IPM instead of 17.70 IPM unless the operator corrects the flute count.
- No calculator behavior or live data was changed. The findings support a bounded correction task before the page is treated as production-ready or “full scope.”

Verification evidence:
- Exact JSON table comparison passed for all calculator data tables.
- Focused feeds/speeds suite passed: 1 file / 6 tests.
- One-off all-family execution covered all 29 tool rows without crashes and exposed the unbounded/specialized outputs above.
- Live `/tools/feeds-speeds` review reproduced the default-flute discrepancy and showed no console errors.
### 2026-07-17 — Quote part-price purchase inclusion and expandable breakdown (latest)
- Corrected the pricing path so a part's marked-up purchased material is included in that part's calculated unit price and lot total rather than appearing only as a detached note or being added twice to the quote estimate.
- Purchase cards now turn from orange `Enter cost` to a calm `Cost captured` state only after the carried vendor and a cost are present; the supporting message confirms that the purchase is included below.
- Replaced the flat price note with an in-tile `View price details` accordion that lists each selected work step, units, rate, priced amount, checklist-only status, and purchased-material math.
- Verification passed: targeted ESLint, TypeScript, focused pricing/quote-total tests (`13/13`), and live quote-editor QA. A $30 cost at 20% markup changed the relevant part from $170.00 to $206.00 and revealed the $36.00 material contribution; the temporary browser-only entry was reloaded without saving.
### 2026-07-17 — Accumulated GitHub publication checkpoint (latest)
- Reviewed and staged the accumulated drawing-to-order, quote-first workflow, pricing, admin, feeds-and-speeds, trusted dispatch, labor history, Read Me First, and Shop Floor work as one product checkpoint.
...
- Removed the separate Kiosk navigation entry and made `/kiosk` redirect to the Shop Floor dashboard.
- Removed kiosk enablement, kiosk status, and kiosk PIN controls from employee administration; primary-department assignment remains available for useful timer defaults.
- The trusted-console timer audit model and Read Me First receipt gate remain unchanged.
- Verification passed: TypeScript, targeted ESLint, diff check, and live browser QA of the dashboard, redirect, navigation, and employee form.

### 2026-07-17 — Authoritative Read Me First timer gate (latest)
- Removed the dispatch timer's bare `instructionsConfirmed` bypass. Starting work now requires a current persisted receipt for the selected employee, part, department, and instruction version.
- The trusted order console shows the complete mission brief before acknowledgement, records the selected employee and signed-in console operator separately, and does not require an employee PIN for each action.
...
### 2026-08-24 — Department workflow, default, and production-layout audit (latest)
- Confirmed the order-detail UI does not expose the backend's arbitrary/manual department-move capability. Its only action submits the current department complete, and the visible next-department preview uses global adjacency while the backend routes to the first department with an open checklist.
- Reproduced the owner's Fab issue live on CRM-1001 / QA-1-1: the part is in Fab, the dialog previews Paint rather than Machining, and the part has no Fab checklist item, so the governed submit path cannot advance it.
- Verified first-department behavior: direct and repeat orders persist the first active department at creation; quote conversion does the same, but a new quote stores the automatic choice as null and resolves the actual department only when converted.
- Documented the repository and Windows production layouts. `C:\ShopApp` has the intended top-level state separation, while the active app is still a full development checkout, config/storage have duplicate in-app paths, `incoming` retains staging artifacts, and recurring backup automation is not installed yet.
- No product or production data changes were made. Focused direct/repeat tests passed: 2 files / 19 tests; authenticated live UI and read-only SSH checks passed.
### 2026-08-25 — Department controls, stable quote origin, and live Order Traveler (latest)
- Restored an explicit audited `Move department` path with destination selection, required reason, active-timer protection, rework flagging, and live error feedback. Kept `Submit department complete` separate, made readiness blockers visible, and shared one checklist-routing helper between UI and service.
- New quotes now select and persist the actual first active department by sort order/name; server creation enforces the same snapshot while existing quotes retain saved origins, including inactive historical choices.
- Replaced the generic order Job Router with an authenticated Order Traveler at `/orders/[id]/print`, linked from every order. It prints one Letter sheet per part with customer/order context, material/specs, required reading, route/checklist/date/initial boxes, notes/files, and final release signoff.
- Verification passed: 28/28 focused tests, targeted ESLint, `npx tsc --noEmit`, clean 62-page production build, authenticated local interaction QA, and Letter-proportioned visual inspection with no browser errors.
- Deployed 14 targeted source files to `192.168.254.72`, retaining source rollback and pre-update DB backups. The server build passed, ShopApp restarted healthy, IP/hostname health and sign-in returned HTTP 200, and live order/move/Traveler QA passed. Database size/last-write and all 12 attachment files were unchanged.

### 2026-08-25 — Production server renamed to SHOPAPP (latest)
- Renamed the Windows 11 production computer from `DESKTOP-BKBAKPM` to `SHOPAPP` and completed the required restart.
- Post-restart verification passed: Windows reports `SHOPAPP`, ShopApp and SSH startup services recovered, Remote Desktop is enabled/listening, and ShopApp health returns HTTP 200 by both `192.168.254.72` and `shopapp.local`.
- The router-reserved address remained `192.168.254.72`; no application, database, or storage changes were made.

### 2026-08-25 — Searchable customer-file mirror, dual health monitoring, and Codex project repair (latest)
- Added a hidden encrypted read-only SMB source on `SHOPAPP`, accessible only from `.10` through a generated mirror identity. Unraid now pulls `C:\ShopApp\storage` into `projects/ShopApp Customer Files` every five minutes without deletions, preserving the existing business/customer/order hierarchy.
- Seeded all 12 customer files. Full source/target SHA-256 comparison and a temporary incremental-copy test passed; the marker was removed and the final mirror returned `changed=0 files=12`.
- Added `ShopApp Health Monitor` on Windows every two minutes with three probes and ShopApp-task-only recovery. Added a separate Unraid HTTP monitor every two minutes with persistent status/log files and outage/recovery notifications.
- Corrected Codex on `.72` from its internal context-only folder to the real trusted `ShopApp Production` project at `C:\ShopApp\app`, preserving the old context and a state backup. Added a desktop pairing guide and `shopapp.local` shortcut.
- Verification passed: both Unraid cron entries persist, both Unraid statuses are healthy, the Windows task completed automatically with result `0`, and local/LAN health remained HTTP 200.
### 2026-08-25 — Local demo follow-up batch ready; production deployment held (latest)
- Expanded the approved Shop Floor navy/royal-blue tile treatment from the part selector to the complete order-detail workspace. Shared `order-detail-tile` and `order-detail-inset` surfaces now cover Overview, Notes & Files, Full Order Files, BOM, To-do / Checklist, and Log without changing semantic status/warning colors.
- Clarified order staffing: `assignedMachinistId` is an optional order coordinator, while direct orders and quote conversions accept multiple assigned workers and seed those workers onto every created part. The traveler now prints Coordinator and Assigned worker(s) independently and ignores inactive part assignments.
- Changed the fresh/default Shop Floor sort to `Order.createdAt` descending. Added a compatibility upgrade only for the prior saved default (`dueDate` ascending), preserving deliberate user-customized sorts.
- Added structured customer shipping fields and reusable multi-contact records. Quotes and orders now select a specific customer contact and preserve contact snapshots so historical paperwork does not silently change when the customer record changes.
- Migration `20260825150000_customer_contacts_structured_address_v1` passed disposable/local application; the local database was backed up before application. Focused tests passed 51/51, targeted lint passed, full TypeScript passed, and the production build generated 62 pages.
- Local browser QA confirmed newest-first Shop Floor results, all six order-detail tab surfaces, traveler coordinator/worker separation, Toyota multi-contact editing, structured address fields, and the quote/direct-order contact and multi-worker controls.
- Deployment is intentionally held. No connection or change was made to `.72`; exact release and rollback steps are in `docs/DEPLOYMENT_READY_2026-08-25.md`.
### 2026-08-25 — Business letterhead, disclaimer block, and quote-number reconfirmation ready locally (latest)
- Extended document-template Header blocks with editable business name, two address lines, phone, and email. The editor now uses the three configured businesses instead of a free-form code and supplies photographed Sterling/C&R presets; Powder Coating remains editable where address/phone details are not yet known. All three share `acct.mgr@crmachinefab.com` by default.
- Updated the quote print header to show business letterhead on the left and QUOTE/number/date on the right. Customer/recipient information remains an independent movable block and now supports structured customer address output.
- Added a draggable Disclaimer block with editable heading and multiline message, live preview, and bordered print rendering modeled on the supplied “PLEASE NOTE” reference.
- Re-audited quote creation: the only create route uses the service-owned `DDMMYY-###` generator, starts each local day at `001`, and preserves old-format numbers only when editing historical quotes. The visible year-month-day behavior is from historical records or the older held server build, not the local generator.
- Focused tests passed 13/13; targeted lint, full TypeScript, and the 62-page production build passed. Browser QA covered C&R editor preview, Disclaimer creation/preview, and a rendered C&R quote letterhead.
- Changes remain local and staged with the existing deployment hold. No `.72` connection or production mutation occurred.
### 2026-08-26 — Three business customer workbooks imported live
- Audited `Sterling customers.xlsx`, `CR customer.xlsx`, and `pkpcustomers.xlsx` from `.10/projects`: 234 source rows across five sheets. Skipped only three accounting/control rows (`ACH Customer`, `acctg adj`, `Srvc Cntr Customer`).
- Added optional customer/contact fax storage and a normalized `CustomerBusiness` membership model, plus customer-list/detail badges and fax editing/display. Added an idempotent XLSX importer that performs conservative alias merging, fills only blank existing fields, preserves existing order-linked customers, and emits dry-run/apply reports.
- Dry-run and disposable production-copy application produced 188 new customers, six safe existing-customer merges, 162 contacts, and 229 business memberships; a second run reported zero pending writes.
- Deployed migration `20260826140000_customer_import_business_fax_v1` and the hash-verified importer/UI release to `.72`. Live totals are 195 customers, 169 contacts, 229 memberships, 157 structured addresses, 137 phones, 40 faxes, and 9 emails. Existing 2 orders, 1 quote, and 2 parts match the pre-import backup.
- Focused tests passed 5/5; targeted ESLint, TypeScript, local/server 62-page builds, server-side idempotency verification, and local `.72` health passed. Rollback: `C:\ShopApp\backups\pre-update\customer-import-20260826-093254`.
### 2026-08-26 — Customer relationship dashboard deployed
- Replaced the Customers card-only directory with Shop Floor-style controls: customer/contact/phone/location search, Business and Activity filters, seven relevant sort modes, Ascending/Descending, and Tiles/List views.
- Added one customer-domain dashboard contract with computed active orders, total orders, physical part quantity, order frequency, labor seconds from valid time intervals (including running intervals), contacts, business memberships, and most recent order/time activity. No totals are persisted.
- Desktop Tiles use the approved navy/royal hierarchy; desktop List provides a dense comparison table; phone List uses compact rows instead of a clipped table.
- Verification passed: customer suites 15/15, targeted ESLint, full TypeScript, `git diff --check`, 62-page production build, desktop interaction QA, and `390x844` Tiles/List QA with a 380px document inside a 390px viewport.
- After explicit approval, deployed six exact-hash source files through `ops/Deploy-ShopApp-CustomerDashboard.ps1`. The server 62-page build passed and all deployed hashes match.
- Rollback: `C:\ShopApp\backups\pre-update\customer-dashboard-20260826-095958`. Loopback, IP, and `shopapp.local` health returned HTTP 200; Customers returned the expected authenticated 307 redirect; ShopApp is Running, Health Monitor is Ready/enabled with last result 0, and `shopapp.err.log` is empty.
### 2026-08-26 — Customer Tile Shop Floor visual parity deployed
- Replaced the customer-only solid blue gradient and nested metric boxes with the exact Shop Floor tile vocabulary: `shop-glass`, translucent border/depth, cyan primary title, open two-column facts, and subtle hover lift.
- Preserved all customer metrics, filters, search, sorting, and List layouts.
- Desktop browser QA confirmed visual parity. Phone QA measured a 380px document inside a 390px viewport with no overflow after clipping only the decorative glow.
- Targeted ESLint, full TypeScript, `git diff --check`, and the 62-page build pass.
- After explicit approval, deployed SHA-256 `42D117C8E275E682819E47EA0392AF9DD74D066CE1D65BAF00E8FC387413A565` through `ops/Deploy-ShopApp-CustomerTileParity.ps1`; the server 62-page build passed.
- Rollback: `C:\ShopApp\backups\pre-update\customer-tile-parity-20260826-101737`. Loopback/IP/`shopapp.local` health is HTTP 200, ShopApp is Running, Health Monitor is Ready/enabled with last result 0, and `shopapp.err.log` is empty.
### 2026-08-26 — Customers page canvas/scroll mechanics corrected directly on `.72`
- Owner identified that the Customers filters/tiles moved beneath a nested layer, showed a separate scrollbar, and exposed backing fill around tile corners because `shop-floor-glass` plus `overflow-x-hidden` was placed on `CustomerDashboard` instead of the page wrapper.
- Direct server-only correction moved `shop-floor-glass` to `src/app/customers/page.tsx`, removed the nested canvas/overflow from `customer-dashboard.ui.tsx`, and removed the filter backing panel.
- Server build passed 62 pages. Automated source assertions: page canvas true; nested canvas false; nested overflow false; filter backing false. Loopback and `shopapp.local` health HTTP 200; ShopApp Running; monitor Ready/result 0; error log 0 bytes.
- Rollback: `C:\ShopApp\backups\pre-update\customer-page-mechanics-20260826-104409`. Local product source was intentionally not changed per owner instruction and must be reconciled before the next release.
### 2026-08-26 — Quote customer list truncation fixed directly on `.72`
- Diagnosed the apparent customer-selector scrolling failure as an explicit `take=100` request; because customers are alphabetized, the returned data ended around L.
- Raised the production quote-editor request to 5,000 customers. The 62-page build passed, ShopApp restarted Running, and loopback health returned HTTP 200.
- Rollback: `C:\ShopApp\backups\pre-update\quote-customer-limit-20260826-110834`. Local product source remains intentionally untouched and must be reconciled before the next deployment.
### 2026-08-26 — Direct-order customer list truncation fixed on `.72`
- Raised the production new-order customer request from 100 to 5,000, matching the quote hotfix. The 62-page build passed, ShopApp restarted Running, and loopback health returned HTTP 200.
- Rollback: `C:\ShopApp\backups\pre-update\order-customer-limit-20260826-121101`. Local product source remains intentionally untouched.
### 2026-08-26 — Drawing upload cap raised to 100 on `.72`
- Raised the production ZIP drawing-import cap from 50 to 100 files and updated the Parts upload guidance. The 62-page build passed, ShopApp restarted Running, and loopback health returned HTTP 200.
- Rollback: `C:\ShopApp\backups\pre-update\drawing-upload-100-20260826-130802`. Local product source remains intentionally untouched.
### 2026-08-26 — Production SSH access documented for future Codex sessions
- Added `docs/PRODUCTION_ACCESS.md` and linked it from required-read Agent Context. It records the `.72` host/user/port, explicit Windows OpenSSH paths, workstation key paths and public fingerprint, production layout, connection test, rollback/build/health sequence, and remote-access boundaries.
- No password or private-key material was stored. Key-based access was verified by receiving hostname `SHOPAPP` from `admin@192.168.254.72`.
## 2026-08-26 — Eight-item order-intake reliability batch staged; production hold active

- Reconciled the recent `.72`-only Customer mechanics, 5,000-customer quote/direct-order list limits, and 100-drawing ZIP limit into workstation source before doing new work.
- Direct order intake now keeps Material Ordered / On Hand independently selectable and uses editable string drafts so quantity `1` can be erased and replaced naturally.
- Quote/manual pricing now persists and displays an explicit Each Part (`PER_UNIT`) or Whole Quantity / Lot (`LOT_TOTAL`) basis through editor, totals, detail, and print.
- Quote and direct-order intake now expose customer-specific Add Contact controls that preserve the full contact collection and primary-contact invariant.
- Quote-to-order conversion detects PO numbers from stored PDF attachments, prefills only a blank field, and repeats detection server-side as a conversion fallback.
- Direct-order selected workers are persisted to every created part; Shop Floor projections, filters, cards, summaries, and department feeds fall back to the deduplicated part assignments instead of incorrectly reporting Unassigned.
- Attachment access is now session-authenticated and server-authorized: non-admins receive only part-scoped drawing kinds and are denied PO/quote/invoice/order documents even through direct URLs.
- Drawing intake now asks One-off versus Assembly before upload, applies an assembly multiplier, autosaves review metadata by stable intake context, clears only after successful quote/order save or explicit clear, allows add-and-reuse material creation, derives cut/stock length deterministically, looks up bounded BOM context for SEE BOM drawings, and extracts with bounded concurrency.
- Model decision: retained the existing model for this release while improving deterministic preprocessing and workflow. Official model guidance recommends newer models for more complex work; migrate only after a representative drawing-evaluation set measures accuracy, latency, and cost.
- Verification: Prisma validate/generate passed; targeted ESLint and full TypeScript passed; focused integrated tests passed 14 files / 101 tests; complete tests passed 44 files / 231 tests; production build passed all 62 routes/pages; `git diff --check` passed with line-ending notices only.
- Local browser QA confirmed independently enabled Material Ordered / On Hand, customer-specific Add Contact, quantity replacement from `1` to `92`, One-off/Assembly first-question behavior, assembly count, and the 100-drawing upload limit.
- Release state: not deployed. The `.72` service was not mutated or restarted because the owner's active 92-drawing confirmation session cannot be retroactively protected by the new autosave code. Deploy only after the owner confirms that session is safely saved/finished.
## 2026-08-26 — Order-intake reliability batch deployed to `.72`

- Owner confirmed the large drawing order had been saved before release. Production audit identifies it as `STD-1002` with 85 created parts; current totals are five orders, 89 parts, two quotes, and 195 customers.
- Deployed the locally verified source/schema/migrations using archive SHA-256 `F40ACA3B49C7D3C4A386B062244B9806DF6131B2514E4935B2925C7D3B73435E` and guarded script `ops/Deploy-ShopApp-OrderIntakeReliability.ps1`.
- Rollback snapshot: `C:\ShopApp\backups\pre-update\order-intake-reliability-20260826-155249`; it contains the pre-release database plus every replaced source/schema/migration file.
- The first guarded attempt stopped before touching the app because Windows PowerShell 5.1 lacks `[System.IO.Path]::GetRelativePath`. Replaced it with a compatible validated substring calculation, reconfirmed live health, and reran successfully.
- Server Prisma generation passed; all 40 migrations were present and none pending. The production build completed 62 routes/pages, the task restarted, and internal health returned OK.
- Final checks: `192.168.254.72` health 200, `shopapp.local` health 200, sign-in 200, ShopApp task Running, Health Monitor Ready with result 0, error log 0 bytes, database remained 1,175,552 bytes with its pre-deploy saved-work timestamp, and 12 representative local/server source hashes matched exactly.

## 2026-08-27 — Multi-page drawing packets deployed

- Changed quote/direct-order drawing intake so one multi-page PDF is rasterized page-by-page and independently analyzed instead of becoming one aggregate part.
- Added explicit page roles (`PART_DRAWING`, `BOM`, `COVER`, `OTHER`). BOM/cover pages remain supporting context rather than false parts; uncertain pages remain in human review. Every extracted drawing is labeled with original filename and page number.
- The original packet is stored once as a quote/order-level supporting file, while each accepted page image is the part-level drawing. Existing one-page PDFs, images, ZIP imports, assembly multiplier, BOM routing, autosave, and employee attachment restrictions remain intact.
- Verification passed: targeted ESLint, full TypeScript, `git diff --check`, 44 test files / 233 tests, and local/server 62-route builds. The focused 22-test drawing suite passed again on `.72`, including native three-page PDF rasterization.
- Deployed five exact-hash files after owner approval. Release SHA-256: `BC99AD2387E0DECC634EC82D8597DB48B2ECE60273570E0B58D4767FF96B802F`; rollback: `C:\ShopApp\backups\pre-update\order-intake-reliability-20260827-100255`.
- Live state: ShopApp Running, Health Monitor Ready/enabled, loopback and LAN health `{"status":"ok"}`, server error log zero bytes. No schema or data mutation was required; 40 migrations were present with none pending.

## 2026-08-27 — Drawing extraction accuracy regression corrected live

- Reproduced the regression with an opt-in synthetic accuracy gate. The model returned usable title-block fields but a reasonable `documentRole` variant failed the strict enum, causing whole-result fallback to the page filename.
- Added tolerant role unwrapping/normalization, direct high-detail image data URLs (removing temporary file readiness races), 2x PDF rendering, a bottom-right title-block detail crop, native page text, one bounded retry, deterministic temperature, and content-safe failure logging.
- The synthetic OpenAI accuracy gate now recovers known part number, name, material, finish, and role locally and on `.72`. No real manufacturing drawing was sent externally for evaluation.
- Full local suite passed 44 files / 236 tests (one opt-in eval skipped); focused deterministic suite passed 25/25; targeted lint, TypeScript, diff check, and 62-route build passed. Server deterministic + accuracy suites passed 26/26 and its 62-route build passed.
...
- Final production health: ShopApp Running, monitor Ready, loopback/LAN `{"status":"ok"}`, zero-byte error log, and four deployed hashes matched the release.
### 2026-08-27 — Drawing stock dimensions staged locally
- Added finished part thickness and width to the drawing extraction contract, confidence/evidence review flow, quote/order persistence, quote conversion, order editing, travelers, and repeat-order templates.
- Drawing confirmation now shows thickness, width, finished length, cut length, and `Total stock dimensions`; the total is deterministically formatted as thickness × width × (cut length × quantity).
- Quote Material Check and its printable Material Shop Walkdown now show the same final dimensions; related quote/order labels were updated from stock size/length to total stock dimensions.
- Added and locally applied migration `20260827140000_drawing_stock_dimensions_v1`. Existing `stockSize` data remains intact and the new nullable columns are backward compatible.
- Verification: focused 57/57 tests, full 238/238 tests, opt-in synthetic extraction accuracy 1/1, TypeScript, targeted ESLint, `git diff --check`, and the 62-page production build passed.
- This change is staged locally only. It has not been deployed to `.72`.
### 2026-08-27 — Drawing stock dimensions deployed
- Released finished thickness/width extraction and `thickness × width × (cut length × quantity)` stock dimensions through drawing review, quote/order persistence, quote conversion, repeat orders, Material Check, printed stock walkdown, order editing, and travelers.
- Applied migration `20260827140000_drawing_stock_dimensions_v1` on `.72`; production now reports 41 migrations and an up-to-date schema. The guarded 34-file release used archive SHA-256 `0867780FF608FA49C1F462F05A9929DBA23C2E870105027FF33AB5D0027875CA`.
- Server verification passed: 57/57 focused tests, 1/1 configured-model synthetic drawing accuracy evaluation, 62-route build, 34/34 exact deployed hashes, HTTP 200 through loopback/LAN/`shopapp.local`, ShopApp Running, monitor Ready/result 0, and empty error log.
- Rollback source/database snapshot: `C:\ShopApp\backups\pre-update\order-intake-reliability-20260827-120200`.
### 2026-08-27 — Long mixed-ZIP imports protected from false health recovery
- Root cause was not a Node crash: the CPU-heavy drawing import delayed the same-process health endpoint, and the health monitor forcibly ended the still-running ShopApp task after three misses. Windows had no Node crash report and ShopApp's error log was empty.
- Authorized drawing imports now create a per-request activity marker before multipart parsing and always clean it up in `finally`. The monitor defers destructive recovery only when a fresh marker and the exact standalone ShopApp Node process both exist; stale markers expire after 45 minutes, and a missing runtime still triggers normal recovery.
- Mixed ZIPs now limit the supported drawing files (PDF/PNG/JPG) to 100 while ignoring unrelated files; a separate 1,000-entry scan ceiling retains archive-abuse protection.
- Local/production focused tests passed 30/30; TypeScript, targeted ESLint, and local/server 62-route builds passed. A forced failed-health production simulation returned `busy-import`, preserved PID 8092, and did not restart. Final hashes 4/4, three health endpoints 200, task Running, monitor Ready/result 0, error log empty.
- Release `071F60D2EFFA20382763760F69E108D0A079162F3A10109025D1241882B762F5`; source/database rollback `C:\ShopApp\backups\pre-update\order-intake-reliability-20260827-135601`; monitor rollback `C:\ShopApp\backups\pre-update\zip-import-health-guard-20260827-134907`.
- Live log follow-up identified a second multipart path, `/api/admin/quotes/upload`, used by quote attachments. It now creates the same pre-parse activity marker. Follow-up release `651175E9AEC7F9D35203F50FE96875EAE20AE84E64671D9596214A9671773901` passed 4/4 local/server guard tests, 2/2 hashes, all health endpoints, and empty error log; rollback `C:\ShopApp\backups\pre-update\order-intake-reliability-20260827-140316`.
## 2026-08-27 — Mixed-ZIP runtime hardening deployed without removing stock dimensions

- Kept the finished width/thickness extraction, review, persistence, stock-finding, and print behavior fully intact. No rollback of the drawing stock-dimension feature was performed.
- Reduced native PDF pressure by serializing PDF packet rendering/text preprocessing while preserving four-way model extraction concurrency. Mixed ZIP parsing no longer asks JSZip to inflate every unrelated archive member merely to verify its CRC; supported drawings are still individually size/ratio validated, the 100-drawing limit remains, and ZIP scanning remains capped at 1,000 entries.
- Added durable drawing-import begin/finish logging alongside the existing activity markers. Added a production launcher with a 12 GB Node heap ceiling, fatal/uncaught diagnostic reports, uncaught tracing, and exact process-exit logging.
- Final verification exposed a separate monitor-recovery defect: stopping the scheduled task could leave its Node child orphaned, making Windows report ShopApp `Ready` while an unmanaged process still answered health checks. The monitor now terminates only the exact ShopApp standalone Node process before starting the scheduled task again.
- The old runtime had exited with task result `1`, no Node process, no error-log content, and no crash report before this deployment. The new diagnostics are intended to make any recurrence attributable instead of silent.
- Deployed archive SHA-256 `FB9CE6C7836D532BC4415F5F29E54167841467BBB1CC8AC7320560212E47A590`. Source/database rollback: `C:\ShopApp\backups\pre-update\order-intake-reliability-20260827-141241`; launcher rollback: `C:\ShopApp\backups\pre-update\runtime-launcher-20260827-141419`.
- Verification passed: local TypeScript, targeted ESLint, 32/32 focused tests, and the 62-route production build; server 32/32 focused tests; 5/5 exact release hashes; launcher hash parity; loopback/LAN/`shopapp.local` health HTTP 200; ShopApp Running; monitor Ready/result 0; zero-byte error log; zero stale markers. The real customer mixed ZIP remains the final runtime proof.
- Monitor correction hash `58EFB0671BF7C93E7A71D04F4C9CC0FD7D1F3CBCE1BDBCF28DF37F81ADFA0606`; rollback `C:\ShopApp\backups\pre-update\health-monitor-recovery-20260827-142121`. A controlled dead-runtime recovery returned the task to `Running`, and a subsequent healthy cycle exited 0 while preserving PID `6112`.
- Live owner retry at `18:28 UTC` proved the remaining gap: Node was healthy at PID `6112`, then disappeared while no route-level import event/marker had yet been created. This showed that Next/Node was receiving or buffering the multipart body before the route handler could mark the import, allowing the monitor to classify the temporarily stalled health endpoint as dead.
- Added a persisted five-minute `pre-route-grace` state whenever health is delayed but the exact ShopApp runtime still exists and no route marker exists. A missing runtime still restarts immediately; a route marker still receives the 45-minute import window; a truly unresponsive markerless runtime restarts after the short grace. Deployed hash `682A1D364E961BCD8AC1F30754AFD3361D203A05B532CB71E9FDB2CBD7CB4E6B`; rollback `C:\ShopApp\backups\pre-update\health-monitor-preroute-20260827-143233`.
- Safe server simulation forced the monitor health probe to an unused port while leaving the real runtime intact: result `pre-route-grace`, exit 0, PID `7952` preserved, production health 200, ShopApp Running, monitor Ready/result 0, error log empty. A second real owner retry remains pending.

### 2026-08-27 — Proven PDF reader restored with width/thickness live
- At the owner's direction, stopped the packet/crash-isolation redesign and restored the previously working serial intake shape: store each supported drawing, extract bounded native text from PDFs, and send that text directly to the configured AI model. The active PDF path no longer rasterizes pages, uploads PDFs as vision images, or uses image-worker subprocesses.
- Retained the narrow stock-dimension extension in the same model contract: finished length, finished width, and finished thickness include value/confidence/evidence and remain null rather than guessed when unclear. The 100-drawing mixed-ZIP boundary, progress reporting, review autosave, stored attachments, and downstream dimension persistence remain intact.
- Local and production focused suites passed 34/34; TypeScript, targeted ESLint, and both 62-route builds passed. Production source hash `E728C72E505B677544DF4BC4BCB9C0744730C326FF78416FEBCF3432A03BA41E` matches local.
- One-file release SHA-256 `43AF36D187EFB7FFDD11A4C910FD840F5B1AA59325178124D0CC158BB7225E58`; rollback/database snapshot `C:\ShopApp\backups\pre-update\order-intake-reliability-20260827-155741`. ShopApp Running, monitor Ready, loopback/LAN/`shopapp.local` health OK, and error log empty.

### 2026-08-27 — Owner-requested drawing-reader rollback complete

- Restored only the drawing-import source files from `order-intake-reliability-20260827-103153`; orders, quotes, customers, uploads, and the SQLite database were preserved.
- The live service hash exactly matches the selected snapshot: `AA16DB0F30A5A6A905E31B2116742CD1BCAF3A452BF0131E58217EB91046C16D`.
- Loopback/LAN health returned `{"status":"ok"}`; ShopApp is Running; the health monitor is enabled; and the temporary build-only type-check bypass was removed before handoff.
- Restored focused suite result: 21 passed and one legacy assertion differs because current shared parts also require thickness/width confirmations. Snapshot source remains unchanged at the owner’s request.

## 2026-08-28 — First real V2 quote-import failure fixed and verified live

- Diagnosed production job `cmtcsbvlm0001x9lcu3462rr4`: the ZIP inventory and retention succeeded, but `PJ-10904-W5500.pdf` failed before page creation or AI with `path` receiving numeric module ID `15754`.
- Confirmed the production bundle had rewritten static `require.resolve('pdfjs-dist/package.json')` into `path.dirname(15754)`. Replaced that resolver with verified source/standalone filesystem candidates and retained copied PDF.js font assets.
- Added a focused PDF runtime regression, a post-build guard that rejects the unsafe numeric bundled resolver, and stage/job stack logging that records no drawing content.
- Verification passed: exact 9-page PDF canonicalize/render test; complete V2 suite 57 passed / 3 opt-in skipped; TypeScript; targeted ESLint; local 63-route build; full durable private ZIP integration to 9 review pages; and real local standalone HTTP upload to `READY_FOR_REVIEW` with 9/9 canonical PDF pages and 11 retained source/support files.
- Deployed four narrow files to `.72` with rollback `C:\ShopApp\backups\pre-update\v2-import-path-fix-20260828-070024`. Server 63-route build passed and the exact ZIP passed an isolated packaged HTTP upload to `READY_FOR_REVIEW`, 9/9 canonical PDFs, 11 supporting files, and no error.
- Final production state: ShopApp Running, monitor Ready, loopback/LAN/`shopapp.local` health 200, production error log 0 bytes, unsafe bundled resolver absent, isolated port 3011 stopped, and all isolated smoke ZIP/database/storage/log copies removed. Real orders, quotes, production database, and customer storage were not changed by the smoke.
# 2026-08-29 — FWD live confirmation walkthrough
- Uploaded the approved FWD ZIP through `shopapp.local` to quote `280826-002`: 16 PDFs, 17 canonical pages, 13 part/assembly pages, and 4 supporting pages at `READY_FOR_REVIEW`.
- Tested quantity autosave, candidate selection, keep-file-only, restore, and page ordering. The test ended restored to 13 part/assembly pages.
- Fixed false confirmation choices from weak embedded text such as `SHEET`; choices now render only for a genuine `conflicting` status. Added `overflow-anchor:none` around dynamic review content.
- Verification: focused Vitest 25/25, TypeScript, ESLint, server 63-route build, health OK, ShopApp Running, monitor result 0, error log 0 bytes.
...
---
# 2026-08-29 — Stable review explanations and unconditional file-only bypass
- Fixed the intermittent explanation collapse: untouched blur no longer saves, and “Why this needs review” uses controlled state that survives rerenders.
- Live browser reproduction focused an untouched Revision input, opened the explanation, waited three seconds, and verified it remained open with the reason visible.
- File-only/reference classification now precedes extraction-failure and part-field validation, so the user never approves material, quantity, dimensions, or other part fields for a drawing kept only as an attachment.
- Focused tests 14/14 plus TypeScript and ESLint passed. Production build/health/task/monitor/log gates passed.
- Latest rollback: `C:\ShopApp\backups\pre-update\drawing-review-simple-20260829-141347`.

---
### 2026-09-02 — Production quote/order workflow and drawing-import latency audit
- Per owner correction, all authoritative code evidence came from a read-only SSH/SCP copy of production `C:\ShopApp\app\src` plus a read-only production database copy. The workstation checkout differs materially in the drawing-import branch and was not used as the final source of truth. No production source, schema, configuration, order, or import job was changed.
- Confirmed two high-impact contract defects in deployed code: quote conversion UI submits editable `parts`, vendor/material/model flags that `ConversionOverrides` does not accept and then rebuilds from persisted quote parts; direct-order create service returns part IDs but `/api/orders` drops them while the client requires them to launch imported BOM analysis.
- Additional workflow risks: order base graph commits before attachments/add-ons/checklists/canonicalization; changing an order customer cannot update/clear its saved contact snapshot; quote/new-order customer lists load 5,000 records while edit loads only 200; there is no optimistic version guard; quote numbering is read-max-plus-one; the three major editors are 2,631–4,170 lines; stale local drafts resume without an explicit age/context decision.
- Production latency proof from the three newest completed Sep 2 single-photo V3 imports: total 41.088–66.356 seconds; pre-AI preparation 11.720–12.003 seconds; one full-page `gpt-5.4-mini` medium-reasoning call 29.310–54.304 seconds (71.3–81.8% of total); post-AI finalization 35–55 ms. The two newest calls emitted 7,092/7,912 output tokens including 6,483/7,300 reasoning tokens, with zero cached input.
- Recommended performance order: benchmark the existing compact structured response/low reasoning on a golden drawing set; fix the page-specific prompt-cache key; retain full-resolution JPEG input while avoiding full-resolution JPEG→PNG→PDF work on the critical path; raise full-page concurrency only for multi-page packets after rate/accuracy testing; separate refinement capacity; permit progressive review; persist merged per-stage timings. Completed jobs currently overwrite `inventoryAndDocumentsMs` with `totalProcessingMs`.
- Live authenticated browser inspection covered quote creation, direct order creation, and order edit at `shopapp.local`. The Chrome extension blocked the supplied file with `fileChooser.setFiles: Not allowed`, so it was not transmitted and no fresh benchmark was claimed. Advancing the quote customer step unexpectedly persisted draft quote `020926-001`; it was left unchanged for owner disposition.
### 2026-09-03 — Quote conversion and order integrity fixes ready for deployment
- Implemented from a fresh production-source download, then reconciled the same focused edits into the dirty workstation checkout without replacing unrelated local work. A second read-only production download confirmed the live-to-staged diff contains only the intended hunks.
- Quote conversion now carries stable source-part identity and persists edited/added/removed parts plus vendor/material/model flags. Source-only drawing metadata and files follow the correct retained part; removed parts no longer shift add-ons or copy unused files. Invalid/duplicate source IDs are rejected before file copying.
- Order-header editing now treats customer/contact as one validated identity: customer changes clear stale contact snapshots, selected contacts are checked against the target customer, and the edit UI reloads the target customer's contact list.
- Direct-order parts, part files, charges, checklist rows, custom values, and initial department are created in one transaction. A failed child write rolls everything back; post-commit file finalization cannot report the order itself as failed and tempt a duplicate retry.
- Added `docs/ORDER_QUOTE_DECOMPOSITION_PLAN.md` with a contract-first, phased decomposition for quote editor, three new-order modes, order detail panels, and order service/repository command families. No dependency or schema migration was added.
- Verification: TypeScript passed; focused ESLint passed with zero output; 78 focused tests passed, including a real disposable-SQLite atomic rollback test; final production build passed all 65 pages and standalone asset copy. Production was not modified because the runbook requires a separate explicit deployment request.
### 2026-09-03 — Order/quote decomposition: first deployment-safe slice
- Executed the first bounded slice of `docs/ORDER_QUOTE_DECOMPOSITION_PLAN.md` against the production-derived staging tree and reconciled the same focused boundaries locally. Production was not changed.
- New-order submission now has explicit direct-order, quote-conversion, and repeat-order clients plus shared intake key/date/number/error helpers. The page continues to own form state and routing while transport contracts are independently testable.
- Extracted the quote progress navigator and controlled order-header editor. The large pages remain coordinators; the next slices are quote step bodies, repeat/direct prefill hooks, and order part/workflow panels.
- Split order header updates and file canonicalization from the service monolith, retaining compatibility exports from `orders.service.ts`. Order creation, query, part/workflow/time, and repository splits remain intentionally queued to avoid a big-bang refactor.
- Verification: `npx tsc --noEmit`; focused ESLint with zero output; focused Vitest 10 files / 85 tests; `npm run build` completed 65 pages plus standalone assets; `git diff --check` passed. No dependency, schema, data, or production change.
### 2026-09-03 — Order/quote decomposition continuation slice
- Isolated repeat-template fetching in a typed intake client while retaining page-owned abort/retry and gating. Added three tests; all shared intake clients pass 10/10.
- Extracted quote new-customer dialog, contact selection/snapshot fields, and part-entry chooser. Extracted the complete selected-part order editor. Parent pages still own API requests, saves, confirmations, navigation, imports, reloads, and toast behavior.
- Consolidated attachment CRUD and canonicalization in `orders.files.service.ts`; moved reusable event recording to `orders.events.service.ts`. Added `orders.files.repo.ts` for ten attachment persistence functions while retaining compatibility exports and the existing real/mock facade.
- Verification: TypeScript PASS; focused ESLint PASS; workflow plus attachment-security regression PASS (17 files / 103 tests); production build PASS (65 pages plus standalone assets); `git diff --check` PASS. Production was not modified.
- Next decomposition seams: quote company/business/custom-field and drawing/manual-part subpanels; new-order quote prefill mapper/controller; order checklist/timer/assignment panels; order create/query/parts/workflow/time service and repo families.
### 2026-09-03 — Order/quote decomposition continuation slice 2
- Moved quote-to-order transport and deterministic mapping into `order-quote-prefill.client.ts`. Mapping preserves source quote part identity, legacy add-ons only on the first part, dimensions, customer/contact, due date, historical conversion notes, and add-on snapshots; three focused tests added.
- Extracted `QuoteManualPartsPanel.tsx` and `PartWorkerAssignmentsPanel.tsx`. Parent coordinators continue owning imports, autosave, network mutations, permission decisions, confirmations, refreshes, and toasts.
- Added `orders.charges.service.ts` and `orders.charges.repo.ts` for the charge command family. Root service wrappers inject current-department/workflow callbacks, avoiding circular imports; root repo compatibility exports preserve the existing real/mock facade.
- Coordinator sizes after this slice: QuoteEditor 3,193 lines; new-order page 2,401; order detail 3,886; order service 2,125; order repo 1,385.
- Verification: TypeScript PASS; focused ESLint PASS; 18 files / 106 workflow, atomicity, conversion, and attachment-security tests PASS; 65-page production build and standalone copy PASS; `git diff --check` PASS. Production unchanged.
- Next: quote drawing-import shell/customer/business/custom-field panels; direct/repeat new-order form panels; order checklist/timer/workflow panels; order create/query/parts/workflow/time service and repository families.

### 2026-09-03 — Order/quote decomposition continuation slice 3
- Extracted controlled `QuoteGeneralInformationCard`, `QuoteCustomIntakeFieldsCard`, `NewOrderCustomerDialog`, and `OrderChecklistPanel` boundaries. Quote/order coordinators retain API calls, persistence, autosave, conversion-mode gating, permissions, confirmation, refresh, and toast behavior.
- Added `orders.parts.service.ts` for add/update/delete commands and `orders.parts.repo.ts` for six Prisma operations. Existing service/repo exports remain compatible, `src/repos/orders.ts` still owns real/mock selection, and workflow/department callbacks avoid service cycles. Material-status update/event transactionality, copied charges, checklist initialization, notes/events, relation cleanup, and workflow sync ordering are preserved.
- Added three focused order-part command tests: copied-charge add sequencing, last-part delete rejection without side effects, and relation-safe deletion receiving every charge before checklist/workflow synchronization. Coordinator sizes are now QuoteEditor 3,117 lines, new-order page 2,281, order detail 3,822, order service 1,934, and order repo 1,223.
- Verification: `npx tsc --noEmit` PASS; focused local/staging ESLint PASS; combined Vitest PASS (19 files / 109 tests); complete local `npm run build` PASS (65 pages plus standalone assets); server-derived staged `npm run build` PASS (66 pages). The initial staged build correctly failed because the selective download lacked Tailwind/PostCSS/Next root configs; those read-only inputs were downloaded from `C:\ShopApp\app` and the rerun passed. Hash parity passed for new local/staging boundary files; no cross-tree staging imports; `git diff --check` PASS.
- Production source, config, database, storage, tasks, and runtime were not modified. Next bounded candidates are new-order header/contact presentation, quote drawing-import shell sections, order timer/workflow panels, or the order query/workflow/time families; continue one seam per verified patch.

### 2026-09-03 — Order/quote decomposition completion push
- Completed every remaining low-coupling presentation seam in the production-derived staging tree and local counterpart. Quote gained drawing entry, purchased items, attachments, totals, build details, routing, custom amounts, and material-check components. New-order intake gained information, entry chooser, drawing entry, parts editor, review, attachments, launch notes, submit, and wizard-control components. Order detail gained overview, files/notes, activity, timer, and status-dialog components.
- Added `orders.create`, `orders.query`, and status-only `orders.workflow` service/repository families plus direct tests, preserving root compatibility exports and the existing real/mock repository facade. Final coordinator sizes: QuoteEditor 2,480 lines; new-order page 1,271 local / 1,272 staged; order-detail page 3,186 local / 3,225 staged; root order service 1,236; root order repo 947.
- Deliberately retained quote work-plan/final-pricing rules and order checklist/department/time lifecycle commands. The quote blocks need one typed pricing controller; order lifecycle needs a typed callback/action contract and transaction-order tests before checklist, transition, and time responsibilities can move without cycles or duplicated rules. Remaining page dialogs also share permission, timer-conflict, confirmation, and refresh state and should move behind focused action hooks.
- Verification passed: `npx tsc --noEmit`; focused local/staged ESLint; focused regression 43 files / 205 tests; local build 65 pages plus standalone assets; production-derived staged build 66 pages plus standalone assets; new-boundary hash parity; no staging cross-tree imports; `git diff --check`. Production remains untouched, so browser smoke is correctly deferred to the deployment runbook.
# 2026-09-03 — Local server synchronization and Chrome workflow regression

- Verified over read-only SSH that the 435 live production source files exactly match the pristine server snapshot. Production remained untouched. Ran the reviewed production-derived implementation locally at `http://127.0.0.1:3100` with an isolated copied database and storage root; `/api/health` returns `ok`.
- Chrome/Playwright completed quote create/edit, quote approval-to-order conversion, direct order creation, repeat-order prefill/creation, order edits, order-detail tabs, status-dialog gating, timer rendering, and a no-save Shop Floor material-status dialog check. Persisted test records were checked directly in the isolated database; no Chrome console errors remained.
- The supplied drawing completed local V2 analysis in 54.586 seconds. `gpt-5.4-mini` medium reasoning consumed 48.862 seconds (89.5%); the rest of the pipeline consumed about 5.724 seconds. Correctly extracted `CK-AC02-S10`, `Grip Pin`, quantity 1, `4140 CR`, 1.00-inch stock/diameter, 4.88-inch finished length, and the grease-fitting note.
- Repaired a local sync omission for Shop Floor part material-status editing by merging only the exact server feature hunks into `ShopFloorLayouts.tsx`, the order dashboard projection, and `orders.types.ts`. Added the matching `/admin/orders` page and characterization test from the server tree. Existing unrelated dirty workstation changes were preserved.
- Repeat template `STD-1005` has a pre-existing dangling PDF path on production. An isolated fixture and corrected copied `AppSettings.attachmentsDir` proved the code copies the physical file correctly; production data was not changed.
- Chrome file upload is still externally blocked until **Allow access to file URLs** is enabled for the ChatGPT Chrome extension. The exact image was therefore submitted to the same local backend route for timing/correctness coverage, not claimed as a Chrome UI upload pass.
- Gates: TypeScript PASS; focused ESLint PASS; 43 files / 205 workflow tests PASS; Shop Floor characterization 6/6 PASS; root production build PASS (65 pages plus standalone assets); staged build PASS (66 pages); health PASS. No deployment.
# 2026-09-03 — Drawing AI medium reasoning and low verbosity

- Added validated `DRAWING_IMPORT_V2_VERBOSITY` configuration with `low` default and wired it into Responses API `text.verbosity` without changing the strict structured-output schema.
- Set both default Terra extraction and focused dimension-refinement reasoning to `medium`; `.env.example` now documents medium reasoning and low verbosity. The first restart omitted the independent availability flag and correctly returned disabled. It was immediately corrected with `DRAWING_IMPORT_V2_MODE=admin_beta` plus V3 enabled; `/api/admin/drawing-import` now returns enabled V3 and Chrome shows the active import panel at `http://127.0.0.1:3100`.
- Verification: root drawing-AI tests 15/15 PASS; server-derived tests 16/16 PASS; root/staged TypeScript PASS; focused ESLint PASS; root production build PASS (65 pages plus standalone assets); local health PASS. The initial unscoped Vitest command also discovered obsolete `.tmp` snapshots, so authoritative reruns used `--dir` scoping. Production was not changed.
### 2026-09-03 — Current importer shares local AI profile; verification partially blocked
- Updated the active server-derived legacy importer behind Use current importer and the root active request to reuse validated Drawing Import AI config. Local profile is gpt-5.4-mini, standard/low reasoning, medium verbosity, concise summary, 10,000 output tokens. Root active request no longer sends temperature=0. No production changes, restart, new dependencies, or database writes.
- Added drawing-import-ai-request.ts plus 16 mocked regression cases per tree. Completed-response guard rejects incomplete/empty/failed responses (including parseable partial JSON), keeps manual review, and adds a specific output-token-limit warning. Existing photo API file cleanup remains in finally.
- Deliberately preserved legacy prompt, json_object output contract, PDF text/packet preprocessing, attachments, routing, and review/saw math. This is settings parity, not identical V3/Playground extraction; no 11-second timing claim. Both DrawingImportPanel.tsx hashes unchanged (staged 0E8F294C0D981CFD445D4E5B69D2E24CD408343737043EB6BBD7BD3404ED6D66; root 565625A7F19A57E66FAE44C09BE78B4CC69584D6BB0FE3C79AD3D9F3AD05691C).
- Passed: root and staged tsc --noEmit, targeted ESLint. Blocked: root/staged Vitest startup spawn EPERM; requested root escalation rejected by approval service HTTP404. Chrome local quote navigation also rejected by same approval-service failure. No workaround attempted. Tests were authored/typechecked but not executed, no fresh live AI/interactive UI verification. Completion remains pending these gates.
- Extra issue observed, not changed: existing local stderr reports V3 job cmtlmtpws000glmsp019mjqa7 / PJ-10904-W5500.pdf failed because PDF.js standard-font directory is missing from this runtime. Separate runtime-packaging investigation is needed; this settings-only change does not fix it.
