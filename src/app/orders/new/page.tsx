		...
		<div className="min-h-screen flex items-center justify-center bg-[#0B0F14] text-[#E6EDF3]">
			<form onSubmit={handleSubmit} className="w-full max-w-md space-y-4 bg-[#121821] p-6 rounded-md shadow">
				...
				<label className="block text-sm">
					...
					<div className="mt-2">
						<button type="button" onClick={() => setShowCreateCustomer((s) => !s)} className="text-sm text-[#34D399] underline">
							+ Add customer
						</button>
						{showCreateCustomer && (
							<div className="mt-2 p-2 bg-[#0F1720] rounded">
								<div className="space-y-2">
									<input
										className="w-full p-2 rounded bg-[#1B2430]"
									...
									<div className="flex gap-2">
										<Button
											type="button"
											className="px-3 py-1 bg-[#34D399] text-black"
											onClick={async () => {
										...
										<Button type="button" onClick={() => setShowCreateCustomer(false)} className="px-3 py-1">Cancel</Button>
									</div>
								</div>
							</div>
						)}
				...
				<label className="block text-sm">
					...
					<Textarea className="mt-1 bg-[#1B2430] p-2" value={notes} onChange={(e) => setNotes(e.target.value)} />
				</label>
				...
				<label className="block text-sm">
					...
					<div className="mt-2 grid grid-cols-2 gap-2">
						{checklistItems.map((item) => (
							<label key={item.id} className="proc-row bg-[#0F1720] p-2 shp flex items-center gap-2 rounded-sm">
								<input
									type="checkbox"
									...
									onChange={(e) => {
										setSelectedChecklist((sel) => (e.target.checked ? [...sel, item.id] : sel.filter((id) => id !== item.id)));
									}}
								/>
				...
				<Button type="submit" disabled={loading} className="w-full py-2 bg-[#34D399] hover:opacity-90 text-black font-semibold">
