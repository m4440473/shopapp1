...
export default function Client({ initial }: { initial: any }) {
  ...
  async function save(newData: any) {
    try {
      ...
      const parsed = ChecklistItemUpsert.parse(newData);
      const payload = parsed as any;

      if (editing) {
        const res = await fetchJson<any>('/api/admin/checklist-items/' + editing.id, {
  ...
  const columns = useMemo(() => [{ key:'label', header:'Label' }, { key:'active', header:'Active' }], []);
    ...
    <div className="space-y-3">
      ...
      {(creating || editing) && (
        <DialogForm title={editing ? 'Edit' : 'New'} onClose={() => (setCreating(false), setEditing(null))} onSubmit={async () => { 
          const payload = (function(){const g=(id)=>document.getElementById(id) as HTMLInputElement; return { label: g('label').value.trim(), active: (document.getElementById('active') as HTMLSelectElement).value==='true' };})();
          await save(payload);
        }}>
          
          <div className="space-y-2">
            <label className="text-xs">Label<input id="label" className="w-full px-2 py-1 bg-neutral-900 border border-neutral-700 rounded"/></label>
