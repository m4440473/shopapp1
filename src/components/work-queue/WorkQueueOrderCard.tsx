    ...
    <Card className="h-full border-border/60 bg-card/70 p-4 transition hover:border-primary/60 hover:shadow-lg hover:shadow-primary/10">
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="rounded-lg border border-border/50 bg-muted/10 px-3 py-2">Parts in dept: {order.partsInDeptCount}</div>
          ...
          <div className="rounded-lg border border-border/50 bg-muted/10 px-3 py-2">Status: {order.status}</div>
        </div>

        <div className="space-y-1 text-xs text-muted-foreground">
          {order.parts.map((part) => (
            <div key={part.id} className="flex items-center justify-between rounded-md border border-border/50 bg-background/60 px-2 py-1" title={part.reasonText ?? undefined}>
              <span>{part.partNumber ?? `#${part.id.slice(0, 6)}`} · Qty {part.quantity ?? 0}</span>
