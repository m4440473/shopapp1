...
export { generateNextOrderNumber, findActiveOrderCustomFields, createOrderWithCustomFields } from './orders.create.repo';
export { findOrderWithDetails, listOrders } from './orders.query.repo';
export { createStatusHistoryEntry, findOrderForWorkflowStatus, findOrderStatus, updateOrderStatus } from './orders.workflow.repo';
...
export async function findOrderCustomerIdentity(id: string) {
  return prisma.order.findUnique({
    where: { id },
    select: { customerId: true, customerContactId: true },
  });
}
